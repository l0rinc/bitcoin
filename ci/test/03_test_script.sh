...
if [[ "${RUN_IWYU}" == true ]]; then
  ...
  if [[ "${IWYU_FILTER_MODIFIED:-}" == "1" ]]; then
    mapfile -t MODIFIED < <(git --git-dir="${BASE_ROOT_DIR}/.git_ci_backup" diff --name-only origin/master...HEAD)
    jq 'map(select(.file as $p | $ARGS.positional | any(. as $m | $p | endswith("/" + $m))))' "${BASE_BUILD_DIR}/compile_commands.json" --args "${MODIFIED[@]}" | sponge "${BASE_BUILD_DIR}/compile_commands.json"
  fi
