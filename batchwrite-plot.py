...
def loadtxoutset(dbbatchsize):
    ...
    subprocess.run([BITCOIN_CLI, f"-datadir={DATADIR}", "stop"], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, cwd=BITCOIN_CORE_PATH)
    time.sleep(5)
    ...
    subprocess.run([BITCOIND, f"-datadir={DATADIR}", "-stopatheight=1"], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL, cwd=BITCOIN_CORE_PATH)
    os.remove(DEBUG_LOG)
