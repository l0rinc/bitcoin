...
def plot_results(results):
    ...
    ax1.plot(elapsed1, flush1, color="darkorange", linestyle="-", label=f"Flush Times (dbbatch={dbbatch1})")
    mean_flush1 = round(statistics.mean(flush1))
    ax1.axhline(linewidth=2, y=mean_flush1, color="darkorange", linestyle="--", alpha=0.5, label=f"Mean flush ({dbbatch1})={mean_flush1}ms")

    ax2.plot(elapsed1, mem1, color="dodgerblue", linestyle="-", label=f"Memory (dbbatch={dbbatch1})")
    max_mem1 = max(mem1)
    ax2.axhline(linewidth=2, y=max_mem1, color="dodgerblue", linestyle="--", alpha=0.5, label=f"Max mem ({dbbatch1})={max_mem1}MiB")
