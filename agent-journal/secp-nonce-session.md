...
### Cycle 95 Evidence and Finding

- The focused source review covered the MuSig nonce-generation, public-nonce parsing/serialization, aggregate-nonce parsing/serialization, nonce aggregation and processing, partial signing/verification/aggregation, Schnorr custom nonce handling, ECDH fallback behavior, extrakeys/tweaks, and Bitcoin's `src/musig.cpp` callers. The parser implementations at `src/secp256k1/src/modules/musig/session_impl.h:188-242` decoded into local group elements and only saved the opaque output after complete success; on malformed input they returned `0` without changing an already initialized output object.
- This was a distinct failure-state candidate, not the earlier Cycle 60 nonce-output argument ordering fix or Cycle 72 invalid-session/nonce-reuse cell. The public header promised only the return value for `musig_pubnonce_parse` and `musig_aggnonce_parse`, but the neighboring `musig_partial_sig_parse` explicitly clears its output so a caller that ignores `WARN_UNUSED_RESULT` cannot reuse an old valid signature. The generic x-only parser and MuSig tweak APIs likewise document invalid output on failure. The Bitcoin MuSig wrapper checks both nonce parser returns at `src/musig.cpp:197` and `src/musig.cpp:285`, so no wrapper defect was claimed.
- A disposable C++ ABI probe, `agent-journal/secp_nonce_cycle95_probe.cpp`, generated a valid nonce, copied it into a caller object, parsed malformed public-nonce bytes, and observed `0` plus the unchanged valid object before the fix. It repeated the check for an aggregate nonce using `02` followed by zero bytes; this is distinct from the all-zero aggregate encoding, which is valid infinity and remains accepted. The pre-fix expected-fail run, linked against the pre-change object, exited `134` at the zero-output assertion: `Assertion std::memcmp(&pubnonce, &zero_pubnonce, sizeof(pubnonce)) == 0 failed.` The same probe also confirmed the existing partial-signature parser already zeroed its output.
- The fix clears `*nonce` immediately after required-argument checks in both parsers, before decoding. The public header now states that each output is set to an invalid value when parsing fails. The in-tree `musig_api_tests` regression checks zeroed public and aggregate nonce objects after malformed input, then continues with valid parsing; it leaves the valid all-zero aggregate-infinity case intact.
