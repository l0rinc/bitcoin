...
## Cycle 209 result: validation-block-reorg target and corpus reachability

### Selection and fresh gate

- The exact selector was `shuf -i 0-98 -n 1 -> 50`, selecting Goal 50 (`fuzz-introspector`). It was distinct from Cycle 208's Rust MuSig wrapper evidence cell, so no reroll was required. The dedicated branch is `uber-cycle-209-fuzz-introspector-20260731`.
- The fresh gate fetched `origin master` at `2026-07-31T12:05:36Z`. Start HEAD was `cf3fa191a617e7bd83c74d1efb1762f66ce318ea`, `origin/master` was `67efced1fc83a0b7215cc1513e7c4754fee0f12f`, merge-base was `a2aab6df97d9f3e1186e8c3fc57ad909cc8aef9b`, and start divergence was `1209 42`. The pre-cycle uber-goal state SHA-256 was `9f5d507e67405fa551f7c4dd5414b76511b3421a57bb0d10aac10ad09f0a3064`.
- The gate passed tracked/index status, `git diff --check`, and `git diff --cached --check`. Catalog, prompt, corrected TSV, and protocol hashes were `5c847ef77405df14b7e7e8fa50430d11a71dcbac3d84df66d25a168d1e955ea8`, `10408ad01c000bba65c1fff135cf2d7d92508bf8a8549141e3d6880f7fe0d4ec`, `babfb36e1a64d8b4ad310459306fa2dfdb240d644d731e2b795177f93a68f1cb`, and `954a67b016918eb2d71c17ae78a12b38f014bb47ed32fe45a0b6f307e5002fc0`. Protected PIDs `777094`, `956381`, `1138182`, and `1157959` stayed alive and untouched.
- The current target's last source commit is `3e9d896b186eff20551bdd8ad55fb7b97668c29b` (`validation: check best header after recompute`, 2026-07-07). The pinned QA-assets checkout is `0772287676fdf3fcf87631b383b12442ab48ce75` (2026-06-25); its current `origin/main:fuzz_corpora` tree has 235 target directories but zero `validation_block_reorg` entries. This is a source/QA snapshot age boundary, not evidence that the target source is unregistered.

### Scope and hypothesis

Cycles 195 and 199 covered `txgraph` and `partially_downloaded_block`; Cycle 34 covered the empty-corpus coverage-runner policy; and Cycle 188 covered the separate `validation_load_mempool` fixture. Those cells remain excluded. The selected cell was the newer production-backed `validation_block_reorg` target and its missing-QA-corpus reachability, including the five seeded DAG topologies and the eight post-seed operations at `src/test/fuzz/validation_block_reorg.cpp:560-741`.

The falsifiable hypothesis was that expensive initialization plus no maintained corpus makes high-risk reorg, invalidation, failure-reset, and flush operations effectively unreachable, and that a source-side selector or corpus change would be needed. The expected contract was preserved: every generated block must be processed through production validation, and every selected operation must finish with the target's active-chain, coins-tip, failed-index, best-header, event, and block-index assertions.

### Static and dynamic evidence

- `CallOneOf` selects from eight operation lambdas with `ConsumeIntegralInRange(0, 7)`, consuming from the end of the input. `SeedTopology` similarly selects five topologies from `0` through `4`. The target has explicit cases for genesis reprocessing, full-block processing, header-only processing, `InvalidateBlock`, `ResetBlockFailureFlags`, `PreciousBlock`, disk flush, and explicit `ChainStateFlushed` notification. This gave a deterministic byte-level model instead of treating a low edge count as proof of a blocker.
- The current Clang 19 ASan/UBSan/libFuzzer dispatcher was rebuilt at current HEAD with `cmake --build /data/my_storage/tmp/cycle131-build-libfuzzer --target fuzz -j2`; its SHA-256 was `46ac97d54adadf99fa9422cfabec6109842550dd9bd08f26f163cca074e45a90`. A current empty replay (`seed=20912`, five seconds, `rss_limit_mb=4096`) completed 67 executions, reached 44 target invocations, added 7 units, ran at 5 executions/second, and peaked at 642 MiB. The target function measured 76/156 edges. All five topology lambdas and all eight operation lambdas were seen in this run, but the non-genesis invalidation assertion branch at line 664 was not; the target-specific function report is in `empty-current-5s-coverage.log`.
- A ten-file scratch witness matrix encoded all five topology choices and all eight operations. The key inputs include `01 03 03` for a non-genesis invalidation, `02 04 04` for invalid-branch failure reset, `01 05 03` for `PreciousBlock`, `01 06 03` for a flush, and `07 03` for an explicit flushed notification. The current sanitized replay (`seed=20906`) executed 15 units, reached every outer operation lambda and every topology lambda, and exited 0 with a 624 MiB peak RSS. The expanded target report showed `Chainstate::InvalidateBlock` at 170/368 edges, `ResetBlockFailureFlags` at 40/90, `PreciousBlock` at 74/158, `ProcessNewBlockHeaders` at 55/147, and `ProcessNewBlock` at 52/124; no sanitizer or assertion diagnostic occurred.
- A 30-second mutation run from the witness matrix (`seed=20909`) completed 203 executions, added 94 units, ran at 6 executions/second, and peaked at 629 MiB with no ASan, UBSan, assertion, or process diagnostic. Replaying the resulting corpus with target-specific coverage (`seed=20910`) completed 143 units and reached 43/100 edges in the invalidation operation, 50/117 in failure reset, 38/88 in precious-block handling, and 19/48 in explicit flush notification. The expanded corpus retained both invalidation sub-branches: line 659 had 10 hits and line 664 had 4.
- The witness oracle is mutation-sensitive. A temporary harness mutation changed `if (index->nHeight == 0)` to an always-true condition. After rebuilding, the clean witness `01 03 03` failed at `validation_block_reorg.cpp:658` on `assert(!invalidated)`, exited 77, and libFuzzer preserved `AQMD` (`\001\003\003`) as the crash artifact. The mutation was restored with `apply_patch`, the dispatcher was rebuilt cleanly, and the unmutated witness replay (`seed=20908`) exited 0.

### Verdict and handoff

The missing-QA-corpus hypothesis is **dismissed as a local harness blocker**. The source-level selector is not stuck: deterministic witnesses reach every topology and operation, the fuzzer mutates those witnesses productively, and the production-backed assertions detect a representative invalidation defect. The condition is still an operational corpus-stewardship gap: the current QA-assets snapshot has no target directory, so CI and coverage jobs begin from an expensive empty state. Adding generated seeds to Bitcoin Core source or weakening the target's production construction is not justified; a future QA-assets contribution should carry the target source revision and the minimized witness provenance.

Fuzz Introspector remains unavailable (`command -v fuzz-introspector` found no executable and `python3 -c 'import fuzz_introspector'` raised `ModuleNotFoundError`). The static source/control-flow model, target-specific function-edge reports, witness matrix, and temporary oracle mutation are therefore a bounded fallback, not a full call-depth report. No Bitcoin Core tracked source or permanent test change is justified in this cycle.

Raw artifacts are `/data/my_storage/tmp/cycle209-artifacts/empty-current-5s-coverage.log`, `/data/my_storage/tmp/cycle209-artifacts/witnesses-current.log`, `/data/my_storage/tmp/cycle209-artifacts/witnesses-fuzz-30s.log`, `/data/my_storage/tmp/cycle209-artifacts/witnesses-expanded-coverage.log`, `/data/my_storage/tmp/cycle209-artifacts/witnesses-mutated.log`, and `/data/my_storage/tmp/cycle209-artifacts/witnesses-clean-final.log`. The next distinct queue is to provision Fuzz Introspector or an equivalent LLVM call-depth report, then audit a different high-risk target; do not reopen the compact-block selector, `txgraph`, or this same missing-corpus cell without a changed target/QA revision.
