...
## Cycle 333 (2026-08-04) — REOPENED by new code: SipHash-1-3-UJ conformance

Draw r151 (raw=3980817170312243793 -> #81). Reopen PASS: delta-A added
SipHasher13UJ (crypto: a0ccd4ad17/c2d7931b5c/25bfca06d6 + CCoinsMap
reuse 2beab94896/3bfdcbd7ee) — new crypto with author-generated
vectors, this campaign's exact surface.

HYPOTHESIS: the 1-3-UJ implementation could deviate from its own
documented construction (header doc = executable spec) without any
in-tree oracle noticing, since the pinned vectors were generated by
the same author/implementation.

EXPERIMENT: independent Python reference written ONLY from the header
doc (init constants, Compress1, Compress1Jumbo's rotated pre-XOR
(v0,v1,v2,v3)^=(d1,d2,d3,d0), unpadded finalizer 0x6465646461706e75,
3 finalization rounds), run against all pinned vectors in
src/test/data/siphash.json (/tmp/siphash13uj_ref.py).

RESULT: 64/64 pinned 13uj vectors MATCH the reference; 146/146
siphash24 values match a canonical SipHash-2-4 implementation
(anchors the JSON file's provenance); jumbo-zero-extension structural
invariant (jumbo(d0,0,0,0) == normal(d0)) holds over 1000 random
cases; empty-input edge matches (74fdd8982aa99156 at k=0). Battery
hash_tests green ties the C++ code to the pinned vectors, so
C++ == reference by transitivity. VERDICT: DISMISSED (conformance
confirmed, two independent verifier forms).

FALSE ALARM LESSON (calibration): first run reported 210/210
"mismatches" — my harness parsed the JSON's big-endian hex with
little-endian int.from_bytes; every pair was exactly byte-reversed.
The tell was the empty-input value matching BEFORE the fix.
Rule: anchor harness parsing on one independently-known value first
(canonical paper vectors for 2-4 would have localized it instantly).

Remaining unreviewed surface: corpus-backed fuzz of integer/coins_view
13UJ arms beyond smoke (queued in the ledger).
