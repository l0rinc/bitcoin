...
## Cycle 139 completion

- Selected goal: 73, `network-state-machine`, from the exact post-Cycle-138 draw `shuf -i 0-98 -n 1` -> `73`. Branch: `uber-cycle-139-network-state-machine-20260730`. Start HEAD at the fresh gate was `7453480ab934331f2275a059f7e85647d2f0fa89`; `origin/master` was `9611a356035be531d62bfc40879f388d5dc359c4`; merge-base was `a2aab6df97d9f3e1186e8c3fc57ad909cc8aef9b`; divergence was `1063 40`. The catalog, selector-prompt, and goal-TSV hashes matched the authoritative values. The tracked/index gate was clean, known unrelated untracked artifacts were preserved, and PID `777094` remained untouched.
- Prior Cycle 103 `DynSock` ownership/EOF fixes and Cycle 115 V1 roundtrip, rejected-send, `more`, and direct V2 fallback cells were excluded. This cycle selected the distinct V2 long-message-type byte-domain cell. `CMessageHeader::IsMessageTypeValid()` defines printable message-type bytes as `0x20..0x7e`; the existing `message_header_type_contracts` test independently asserts that a leading `0x7f` DEL byte is invalid. `V2Transport::GetMessageType()` instead used `> 0x7f`, so a remote V2 packet with a long type containing `0x7f` was accepted. The transport fuzzer's message-type generator also stopped at `0x7e`, explaining why its existing property did not cover the mismatch.
- A temporary focused regression was built with `CCACHE_DIR=/data/my_storage/tmp/cycle139-ccache cmake --build /data/my_storage/tmp/cycle89-build --target test_bitcoin -j4`. Before the fix, `TMPDIR=/data/my_storage/tmp/cycle139-net-tests-before /data/my_storage/tmp/cycle89-build/bin/test_bitcoin --run_test=net_tests/transport_v2_rejects_del_message_type --log_level=test_suite --color_output=false` exited `201`: the V2 `Interact()` path returned a non-null message instead of rejecting the fragmented encrypted packet. The test uses the production `V2Transport` and its scenario helper, so the failing input crosses the real transport boundary rather than a parser-only mock.
- The minimal fix changes the V2 upper bound from `0x7f` to `0x7e`. The retained regression exercises a fragmented handshake and application packet, verifies `0x7e` remains accepted, and verifies `0x7f` is rejected. After the fix, the focused regression and the independent `net_tests/message_header_type_contracts` case passed. The complete `net_tests` suite passed 35 cases with `*** No errors detected`.
- Instrumented validation rebuilt `/data/my_storage/tmp/cycle131-build-libfuzzer` from this checkout with its existing address, undefined-behavior, and libFuzzer sanitizers. The exact run `FUZZ=p2p_transport_serialization ASAN_OPTIONS=detect_leaks=0:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 /data/my_storage/tmp/cycle131-build-libfuzzer/bin/fuzz /data/my_storage/tmp/cycle131-runs/libfuzzer-seed -runs=1000 -print_final_stats=1` completed 1,000 executions over 593 corpus files, added 6 units, and emitted no sanitizer diagnostics. Peak RSS was 1,340 MB.
- The full current GCC unit binary started 1,218 cases but exited `201` on the unrelated `validation_block_tests/processnewblock_signals_ordering` assertion (`!sub->m_update_start_tip`); it also emitted an expected filesystem-fault diagnostic from another test. This does not overlap the network change. No other goal-73 focused test failed.
- Verdict: confirmed protocol/input-validation mismatch, fixed with one source line and one focused regression. A remote V2 peer could deliver a DEL-containing message type that V1 rejects; current message processing would treat it as an unknown type, so no direct memory-safety, consensus, or state-corruption impact was established. The change restores the shared message-type contract. Limitations are x86_64 Linux for the direct unit run, no public network, and no Windows, ARM, 32-bit, or big-endian socket execution. The sanitized fuzzer corpus was an existing general transport corpus, not a retained DEL-specific seed.
