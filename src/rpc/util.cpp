...
UniValue RPCMethod::HandleRequest(const JSONRPCRequest& request) const
{
    ...
    if (request.mode == JSONRPCRequest::GET_HELP || !IsValidNumArgs(request.params.size())) {
        if (request.mode == JSONRPCRequest::GET_HELP && request.strMethod == "format" && request.params.isArray() && request.params.size() > 1) {
            throw std::runtime_error(ToString(request.params[1].get_str()));
        }
...
std::string RPCMethod::ToStringArgsCli() const
{
    std::string res;
    for (const auto& arg : m_args) {
...
std::string RPCMethod::ToString(const std::string& format) const
{
    if (format == "default") {
