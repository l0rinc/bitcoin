    ...
    EXCLUSIVE_LOCKS_REQUIRED(!m_mutex)
{
    ...
    if (const auto it{std::ranges::max_element(
        ...
        auto& [tx, sent_to]{*it};
        sent_to.emplace_back(will_send_to_nodeid, Now<NodeSeconds>(), std::nullopt);
        return tx;
    }
    ...
    EXCLUSIVE_LOCKS_REQUIRED(!m_mutex)
{
    LOCK(m_mutex);
    if (const auto tx_and_status{GetSendStatusByNode(nodeid)}) {
        tx_and_status->send_status.confirmed = Now<NodeSeconds>();
    }
}
...
{
    LOCK(m_mutex);
    const auto stale_time{Now<NodeSeconds>() - STALE_DURATION};
