    ...
    EXCLUSIVE_LOCKS_REQUIRED(!m_mutex)
{
    ...
    const auto it{std::ranges::max_element(
            m_transactions,
            [](const Priority& a, const Priority& b) { return a < b; },
            [](const auto& el) { return DerivePriority(el.second); })};

    if (it != m_transactions.end()) {
        auto& [tx, sent_to]{*it};
        sent_to.push_back(SendStatus{.nodeid = will_send_to_nodeid,
                                     .picked = NodeClock::now(),
    ...
    for (auto& [tx, sent_to] : m_transactions) {
        for (auto& send_status : sent_to) {
            if (send_status.nodeid == nodeid) {
                return TxAndSendStatusForNode{.tx = const_cast<CTransactionRef&>(tx), .send_status = send_status};
            }
        }
    }
    return std::nullopt;
