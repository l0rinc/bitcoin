...
protected:
    bool ActivateBestChainStep(BlockValidationState& state, CBlockIndex& index_most_work, bool& fInvalidFound, std::vector<ConnectedBlock>& connected_blocks) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs);
    bool ConnectTip(
        BlockValidationState& state,
