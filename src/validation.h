...
/** Maximum number of worker threads and queued blocks allowed for block read-ahead */
...
protected:
    class BlockFetcher;
    bool ActivateBestChainStep(BlockValidationState& state, CBlockIndex& index_most_work, BlockFetcher& fetcher, bool& fInvalidFound, std::vector<ConnectedBlock>& connected_blocks) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool->cs);
    bool ConnectTip(
        BlockValidationState& state,
