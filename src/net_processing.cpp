...
void PeerManagerImpl::PushPrivateBroadcastTx(CNode& node)
{
    ...
    if (!opt_tx) {
        LogDebug(BCLog::PRIVBROADCAST, "Disconnecting: no more transactions for private broadcast (connected in vain), %s", node.LogPeer(fLogIPs));
        node.fDisconnect = true;
        return;
    }
    const CTransactionRef& tx{*opt_tx};

    LogInfo("[privatebroadcast] P2P handshake completed, sending INV for txid=%s%s, %s",
            tx->GetHash().ToString(), tx->HasWitness() ? strprintf(", wtxid=%s", tx->GetWitnessHash().ToString()) : "",
            node.LogPeer(fLogIPs));
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        LogDebug(BCLog::NET, "receive version message: %s: version %d, blocks=%d, us=%s, txrelay=%d, %s%s\n",
                  cleanSubVer.empty() ? "<no subver>" : cleanSubVer, pfrom.nVersion,
                  peer->m_starting_height, addrMe.ToStringAddrPort(), fRelay, pfrom.LogPeer(fLogIPs),
        ...
        if (pfrom.IsPrivateBroadcastConn()) {
            if (fRelay) {
                ...
                LogInfo("[privatebroadcast] Disconnecting: does not support transactions relay (connected in vain), %s",
                        pfrom.LogPeer(fLogIPs));
                pfrom.fDisconnect = true;
            }
            return;
        }
    ...
    if (pfrom.IsPrivateBroadcastConn()) {
        if (msg_type != NetMsgType::PONG && msg_type != NetMsgType::GETDATA) {
            LogDebug(BCLog::PRIVBROADCAST, "Ignoring incoming message '%s', %s", msg_type, pfrom.LogPeer(fLogIPs));
            return;
        }
    }
    ...
    if (msg_type == NetMsgType::GETDATA) {
        ...
        if (pfrom.IsPrivateBroadcastConn()) {
            ...
            if (!pushed_tx_opt) {
                LogInfo("[privatebroadcast] Disconnecting: got GETDATA without sending an INV, %s",
                        pfrom.LogPeer(fLogIPs));
                pfrom.fDisconnect = true;
                return;
            }
            ...
            if (vInv.size() == 1 && vInv[0].IsMsgTx() && vInv[0].hash == pushed_tx->GetHash().ToUint256()) {
                ...
                LogInfo("[privatebroadcast] Disconnecting: got an unexpected GETDATA message, %s",
                        pfrom.LogPeer(fLogIPs));
                pfrom.fDisconnect = true;
            }
            return;
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (const auto num_broadcasted{m_tx_for_private_broadcast.Remove(ptx)}) {
            LogInfo("[privatebroadcast] Received our privately broadcast transaction (txid=%s) from the "
                    ...
                    txid.ToString(), pfrom.LogPeer(fLogIPs));
                ...
                if (nonce == peer->m_ping_nonce_sent) {
                    ...
                    if (ping_time.count() >= 0) {
                        ...
                        if (pfrom.IsPrivateBroadcastConn()) {
                            ...
                            LogInfo("[privatebroadcast] Got a PONG (the transaction will probably reach the network), marking for disconnect, %s",
                                    pfrom.LogPeer(fLogIPs));
                            pfrom.fDisconnect = true;
                        }
                    } else {
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
    ...
    if (pto->IsPrivateBroadcastConn()) {
        if (pto->m_connected + PRIVATE_BROADCAST_MAX_CONNECTION_LIFETIME < current_time) {
            LogInfo("[privatebroadcast] Disconnecting: did not complete the transaction send within %d seconds, %s",
                    count_seconds(PRIVATE_BROADCAST_MAX_CONNECTION_LIFETIME), pto->LogPeer(fLogIPs));
            pto->fDisconnect = true;
        }
        return true;
    }
