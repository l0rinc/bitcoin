...
private:
    ...
    */
    ...
    /** As above, but the message carries `reservation` until it has been sent. */
    template <typename... Args>
    void MakeAndPushMessage(CNode& node, ResponseMemoryReservation&& reservation, std::string msg_type, Args&&... args) const
    {
    ...
    */
    ...
    /** Return false when global response memory is exhausted and the request should be retried. */
...
bool PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv)
{
    ...
    if (a_recent_block && a_recent_block->GetHash() == inv.hash) {
        ...
        if (const auto block_data{m_chainman.m_blockman.ReadRawBlock(block_pos)}) {
            MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::BLOCK, std::span{*block_data});
        } else {
    ...
    if (pblock) {
        if (inv.IsMsgBlk()) {
            MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::BLOCK, TX_NO_WITNESS(*pblock));
        } else if (inv.IsMsgWitnessBlk()) {
            MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::BLOCK, TX_WITH_WITNESS(*pblock));
            ...
            if (sendMerkleBlock) {
                if (merkleBlock.vMatchedTxn.empty()) {
                    MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::MERKLEBLOCK, merkleBlock);
                } else {
                    ...
                    for (auto it{merkleBlock.vMatchedTxn.begin()}; it != merkleBlock.vMatchedTxn.end(); ++it) {
                        const auto& [tx_idx, _] = *it;
                        if (std::next(it) == merkleBlock.vMatchedTxn.end()) {
                            MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::TX, TX_NO_WITNESS(*pblock->vtx[tx_idx]));
                        } else {
            ...
            if (can_direct_fetch && pindex->nHeight >= tip->nHeight - MAX_CMPCTBLOCK_DEPTH) {
                if (a_recent_compact_block && a_recent_compact_block->header.GetHash() == inv.hash) {
                    MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::CMPCTBLOCK, *a_recent_compact_block);
                } else {
                    CBlockHeaderAndShortTxIDs cmpctblock{*pblock, m_rng.rand64()};
                    MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::CMPCTBLOCK, cmpctblock);
                }
            } else {
                MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::BLOCK, TX_WITH_WITNESS(*pblock));
            }
...
void PeerManagerImpl::ProcessGetData(CNode& pfrom, Peer& peer, const std::atomic<bool>& interruptMsgProc)
{
    ...
    while (it != peer.m_getdata_requests.end() && it->IsGenTxMsg()) {
        ...
        if (auto tx{FindTxForGetData(*tx_relay, ToGenTxid(inv))}) {
            ...
            MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::TX, maybe_with_witness(*tx));
    ...
    if (it != peer.m_getdata_requests.end() && !pfrom.fPauseSend) {
        ...
        if (inv.IsGenBlkMsg()) {
            ...
            // If the first item on the queue is an unknown type, erase it and continue processing the queue on the next call.
...
void PeerManagerImpl::SendBlockTransactions(CNode& pfrom, Peer& peer, const CBlock& block, const BlockTransactionsRequest& req)
{
    ...
    MakeAndPushMessage(pfrom, std::move(response_memory), NetMsgType::BLOCKTXN, resp);
}
...
bool PeerManagerImpl::ProcessMessages(CNode& node, std::atomic<bool>& interruptMsgProc)
{
    ...
    {
        LOCK(peer.m_getdata_requests_mutex);
        if (!peer.m_getdata_requests.empty()) return !peer.m_getdata_blocked_by_global_memory;
    }
