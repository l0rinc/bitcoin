...
static void ValidateIncludePath(kj::StringPtr path)
{
    if (path.size() == 0) {
        throw std::runtime_error("include annotation has an empty path");
    }
    for (size_t i = 0; i < path.size(); ++i) {
        const unsigned char c = path[i];
        if (c == '"' || c == '\\' || c < 0x20 || c == 0x7f) {
            throw std::runtime_error("include annotation contains an invalid path character");
        }
    }
}
    ...
    const std::vector<kj::Own<const kj::ReadableDirectory>>& import_dirs)
{
    ...
    for (const auto annotation : file_schema.getProto().getAnnotations()) {
        if (annotation.getId() == INCLUDE_ANNOTATION_ID ||
            annotation.getId() == INCLUDE_TYPES_ANNOTATION_ID) {
            ValidateIncludePath(annotation.getValue().getText());
        }
    }
