...
public:
    ...
    private:
        ...
        explicit Handle(CoinsViewCacheAsync& cache LIFETIMEBOUND, const CBlock& block LIFETIMEBOUND) noexcept
            : m_cache{cache}
        {
            m_cache.StartFetching(block);
        }
    ...
    [[nodiscard]] Handle StartFetching(const CBlock& block LIFETIMEBOUND) noexcept
    {
        return Handle{m_cache, block};
    }

    void SetBackend(CCoinsView& view_in) noexcept { m_cache.SetBackend(view_in); }
};
