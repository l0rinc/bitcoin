...
private:
    ...
    /** Map of txids to information about that transaction. Only ever looked up by txid, so it is
     *  hashed rather than sorted; every key is the txid of a transaction accepted to the mempool
     *  and therefore one we computed ourselves. */
