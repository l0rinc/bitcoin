...
unsigned int GetP2SHSigOpCount(const CTransaction& tx, const CCoinsViewCache& inputs)
{
    ...
    {
        const Coin& coin = inputs.AccessCoin(tx.vin[i].prevout, key_buffer);
        assert(!coin.IsSpent());
...
bool Consensus::CheckTxInputs(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, Span<std::byte> key_buffer)
{
    // are the actual inputs available?
    if (!inputs.HaveInputs(tx, key_buffer)) {
