...
bool V1Transport::SetMessageToSend(CSerializedNetMsg& msg) noexcept
{
    ...
    Assume(msg.m_type.size() <= CMessageHeader::MESSAGE_TYPE_SIZE);
...
void CConnman::ThreadDNSAddressSeed(bool use_seednodes)
{
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    if (!gArgs.GetBoolArg("-dnsseed", DEFAULT_DNSSEED))
        ...
        threadDNSAddressSeed = std::thread(&util::TraceThread, "dnsseed", [this, use_seednodes = !seed_nodes.empty()] { ThreadDNSAddressSeed(use_seednodes); });
