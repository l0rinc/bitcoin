...
CKey RandKey(FastRandomContext& rng)
{
    CKey k;
...
namespace benchmark {
DataStream GetBlockData(const CChainParams& chain_params, const ScriptRecipe& recipe, uint256 seed)
{
    return SerializeBlock(BuildBlock(chain_params, recipe, seed));
}

CBlock GetBlock(const CChainParams& chain_params, const ScriptRecipe& recipe, uint256 seed)
{
