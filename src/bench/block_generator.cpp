...
namespace benchmark {
DataStream GetBlockData(const std::unique_ptr<const CChainParams>& chainParams, const ScriptRecipe& recipe)
{
    return SerializeBlock(BuildBlock(*chainParams, recipe));
}

CBlock GetBlock(const std::unique_ptr<const CChainParams>& chainParams, const ScriptRecipe& recipe)
{
