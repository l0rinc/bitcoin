...
#include <bench/data/block784588.raw.h>
...
{
    DataStream stream(benchmark::data::block784588);
    std::byte a{0};
    ...
    bench.unit("block").run([&] {
        ...
        bool rewound = stream.Rewind(benchmark::data::block784588.size());
        assert(rewound);
    });
}
...
{
    CBlock block;
    DataStream(benchmark::data::block784588) >> TX_WITH_WITNESS(block);
