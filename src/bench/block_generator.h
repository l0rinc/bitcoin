...
DataStream GetCustomBlockData(
    const std::unique_ptr<const CChainParams>& chainParams = CChainParams::RegTest(CChainParams::RegTestOptions{}),
    const ScriptRecipe& = kWitness
);
CBlock GetCustomBlock(
    const std::unique_ptr<const CChainParams>& chainParams = CChainParams::RegTest(CChainParams::RegTestOptions{}),
    const ScriptRecipe& = kWitness);
} // namespace benchmark
