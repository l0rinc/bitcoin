...
BOOST_AUTO_TEST_CASE(passphrase_rounds_limit) {
    ...
    CCrypter crypt;
    // Counts outside the signed KDF loop domain must be rejected before narrowing.
    for (unsigned int rounds : {0u, static_cast<unsigned int>(std::numeric_limits<int>::max()) + 1u}) {
