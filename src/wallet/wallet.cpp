...

    {
        ...
        for (MasterKeyMap::value_type& pMasterKey : mapMasterKeys)
        {
            ...
            if (Unlock(_vMasterKey))
            {
                ...
                pMasterKey.second.nDeriveIterations = ScaleCryptoRounds(pMasterKey.second.nDeriveIterations, target, SteadyClock::now() - start);

                start = SteadyClock::now();
                crypter.SetKeyFromPassphrase(strNewWalletPassphrase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivationMethod);
                pMasterKey.second.nDeriveIterations = (pMasterKey.second.nDeriveIterations + ScaleCryptoRounds(pMasterKey.second.nDeriveIterations, target, SteadyClock::now() - start)) / 2;
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
    ...
    kMasterKey.nDeriveIterations = ScaleCryptoRounds(25000, target, SteadyClock::now() - start);

    start = SteadyClock::now();
    crypter.SetKeyFromPassphrase(strWalletPassphrase, kMasterKey.vchSalt, kMasterKey.nDeriveIterations, kMasterKey.nDerivationMethod);
    kMasterKey.nDeriveIterations = (kMasterKey.nDeriveIterations + ScaleCryptoRounds(kMasterKey.nDeriveIterations, target, SteadyClock::now() - start)) / 2;
