...

    {
        ...
        for (auto& [master_key_id, master_key] : mapMasterKeys)
        {
            ...
            if (Unlock(plain_master_key))
            {
                ...
                const bool written{WalletBatch(GetDatabase()).WriteMasterKey(master_key_id, new_master_key)};
                if (written) master_key = new_master_key;
                if (fWasLocked)
                    Lock();
                return written;
            }
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        if (!encrypted_batch->WriteMasterKey(nMasterKeyMaxID + 1, master_key)) {
