...
public:
    ...
    /** Get the txid and wtxid of all children that spend from this tx and were received from nodeid. Sorted
     * reconsiderable before non-reconsiderable, then from most recent to least recent. */
    virtual std::vector<std::pair<Txid, Wtxid>> GetChildrenFromSamePeer(const CTransactionRef& parent, NodeId nodeid) const = 0;
