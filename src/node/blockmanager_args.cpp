...
util::Result<uint64_t> ParsePruneOption(const int64_t nPruneArg, const std::string_view opt_name)
{
    ...
    const uint64_t nPruneTarget{uint64_t(nPruneArg) * 1_MiB};
    if (nPruneTarget < MIN_DISK_SPACE_FOR_BLOCK_FILES) {
        return util::Error{strprintf(_("%s configured below the minimum of %d MiB.  Please use a higher number."), opt_name, MIN_DISK_SPACE_FOR_BLOCK_FILES / 1_MiB)};
    }
    return nPruneTarget;
}
...
util::Result<void> ApplyArgsManOptions(const ArgsManager& args, BlockManager::Options& opts)
{
    ...
    if (const auto prune_during_init{args.GetIntArg("-pruneduringinit")}) {
        if (*prune_during_init == -1) {
            ...
            opts.prune_target_during_init = *prune_parsed > uint64_t{std::numeric_limits<int64_t>::max()} ?
                std::numeric_limits<int64_t>::max() :
                static_cast<int64_t>(*prune_parsed);
        } else {
            return util::Error{util::ErrorString(prune_parsed)};
        }
    }
