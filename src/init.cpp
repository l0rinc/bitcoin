...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-dbfilesize",
                   strprintf("Target size of files within databases, in MiB (%u to %u, default: %u).",
                             1, 1024,
                             static_cast<unsigned>(DBWRAPPER_MAX_FILE_SIZE / 1_MiB)),
    ...
    argsman.AddArg("-lowmem=<n>", strprintf("If system available memory falls below <n> MiB, flush caches (0 to disable, default: %s)", g_low_memory_threshold / 1_MiB), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-datacarriersize",
                   strprintf("Maximum size of data in data carrier transactions we relay and mine, in bytes (maximum %u, default: %u)",
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    {
        std::string strSpkReuse = args.GetArg("-spkreuse", DEFAULT_SPKREUSE);
        // Uses string values so future versions can implement other modes
        if (strSpkReuse == "allow" || args.GetBoolArg("-spkreuse", false)) {
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (status == ChainstateLoadStatus::FAILURE && !do_reindex && !ShutdownRequested(node)) {
        ...
        // suggest a reindex
            ...
            LogInfo("Automatically running a reindex.");
            do_retry = true;
        }
        if (!do_retry) {
            return false;
    ...
    if (args.IsArgSet("-blocknotify")) {
        ...
        uiInterface.NotifyBlockTip.connect([blocknotify_commands](SynchronizationState sync_state, const CBlockIndex& block, double /* verification_progress */) {
            if (sync_state != SynchronizationState::POST_INIT) return;
            const std::string blockhash_hex = block.GetBlockHash().GetHex();
...
#ifdef ENABLE_TOR_SUBPROCESS
        std::string tor_execute{gArgs.GetArg("-torexecute", DEFAULT_TOR_EXECUTE)};
        if (tor_execute == "1") {
            tor_execute = DEFAULT_TOR_EXECUTE;
        } else if (tor_execute == "0") {
            tor_execute.clear();
        }
#else
        const std::string tor_execute;
#endif
        node.tor_controller = std::make_unique<TorController>(gArgs.GetArg("-torcontrol", DEFAULT_TOR_CONTROL), onion_service_target, tor_execute);
    }
    ...
    if (g_software_expiry) {
        scheduler.scheduleFromNow([&node] {
            ...
            uiInterface.ThreadSafeMessageBox(msg, CClientUIInterface::ICON_WARNING | CClientUIInterface::BTN_OK);
