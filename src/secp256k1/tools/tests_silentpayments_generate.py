...
#define MAX_RECEIVE_SUBTESTS {MAX_RECEIVE_SUBTESTS}
...
def gen_test_vectors(test_vectors):
    ...
    for test_i, test_vector in enumerate(test_vectors):
        ...
        # its emitted data structures and the test code consuming the generated header would need to
        ...
        assert len(test_vector['receiving']) <= MAX_RECEIVE_SUBTESTS
        out += f"        {len(test_vector['receiving'])}, /* number of receive subtests */\n"
        out += "        {\n"
        for recv_test in test_vector['receiving']:
            ...
            out += "        },\n"
        for _ in range(len(test_vector['receiving']), MAX_RECEIVE_SUBTESTS):
            out += '        { "", "", 0, {"",}, 0, {0,}, 0, 0, {"",}, {"",}, {"",} }, /* unused receive subtest entry */\n'
        out += "        },\n"
        out += "    },\n\n"
    out += "};\n"
    return out
