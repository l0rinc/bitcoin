...
void CCoinsView::BatchWrite(CoinsViewCacheCursor& cursor, const uint256& hash_block)
{
...
void CCoinsViewBacked::SetBackend(CCoinsView& view_in) { base = &view_in; }
void CCoinsViewBacked::BatchWrite(CoinsViewCacheCursor& cursor, const uint256& hash_block) { base->BatchWrite(cursor, hash_block); }
std::unique_ptr<CCoinsViewCursor> CCoinsViewBacked::Cursor() const { return base->Cursor(); }
size_t CCoinsViewBacked::EstimateSize() const { return base->EstimateSize(); }

CCoinsViewCache::CCoinsViewCache(CCoinsView* base_in, bool deterministic) :
...
{
    hashBlock = hash_block_in;
}

void CCoinsViewCache::BatchWrite(CoinsViewCacheCursor& cursor, const uint256& hash_block_in)
{
    for (auto it{cursor.Begin()}; it != cursor.End(); it = cursor.NextAndMaybeErase(*it)) {
