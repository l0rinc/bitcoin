...
void CCoinsViewCache::AddCoin(const COutPoint &outpoint, Coin&& coin, bool possible_overwrite) {
    ...
    if (!inserted) {
        m_dirty_count = CheckedSub(m_dirty_count, it->second.IsDirty());
        cachedCoinsUsage = CheckedSub(cachedCoinsUsage, it->second.coin.DynamicMemoryUsage());
    }
    ...
    for (auto it{cursor.Begin()}; it != cursor.End(); it = cursor.NextAndMaybeErase(*it)) {
        ...
        if (inserted) {
            ...
            if (itUs->second.IsFresh() && it->second.coin.IsSpent()) {
                ...
                m_dirty_count = CheckedSub(m_dirty_count, itUs->second.IsDirty());
                cachedCoinsUsage = CheckedSub(cachedCoinsUsage, itUs->second.coin.DynamicMemoryUsage());
