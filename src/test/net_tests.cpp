...
BOOST_AUTO_TEST_CASE(cnetaddr_basic)
{
    ...
    CHECK(addr.SetSpecial(torv3_addr));
    CHECK(addr.IsValid());
    CHECK(addr.IsTor());
    ...
    CHECK(addr.SetSpecial(i2p_addr));
    CHECK(addr.IsValid());
    CHECK(addr.IsI2P());
    ...
    CHECK(!addr.IsValid()); // "internal" is considered invalid
...
BOOST_AUTO_TEST_CASE(cnetaddr_unserialize_v2)
{
    ...
    CHECK(!s.empty()); // The stream is not consumed on invalid input.
    s.clear();
...
BOOST_AUTO_TEST_CASE(advertise_local_address)
{
    ...
    CHECK(addr_ipv4.IsValid());
    CHECK(addr_ipv4.IsIPv4());

    CAddress addr_ipv6{Lookup("1122:3344:5566:7788:9900:aabb:ccdd:eeff", 8333, false).value(), NODE_NONE};
    CHECK(addr_ipv6.IsValid());
    CHECK(addr_ipv6.IsIPv6());

    CAddress addr_ipv6_tunnel{Lookup("2002:3344:5566:7788:9900:aabb:ccdd:eeff", 8333, false).value(), NODE_NONE};
    CHECK(addr_ipv6_tunnel.IsValid());
    CHECK(addr_ipv6_tunnel.IsIPv6());
    CHECK(addr_ipv6_tunnel.IsRFC3964());

    CAddress addr_teredo{Lookup("2001:0000:5566:7788:9900:aabb:ccdd:eeff", 8333, false).value(), NODE_NONE};
    CHECK(addr_teredo.IsValid());
    CHECK(addr_teredo.IsIPv6());
    CHECK(addr_teredo.IsRFC4380());

    CAddress addr_onion;
    CHECK(addr_onion.SetSpecial("pg6mmjiyjmcrsslvykfwnntlaru7p5svn6y2ymmju6nubxndf4pscryd.onion"));
    CHECK(addr_onion.IsValid());
    CHECK(addr_onion.IsTor());

    CAddress addr_i2p;
    CHECK(addr_i2p.SetSpecial("udhdrtrcetjm5sxzskjyr5ztpeszydbh4dpl3pl4utgqqw2v4jna.b32.i2p"));
    CHECK(addr_i2p.IsValid());
    CHECK(addr_i2p.IsI2P());
    ...
    CHECK(addr_cjdns.IsValid());
    CHECK(addr_cjdns.IsCJDNS());
...
public:
    ...
    void ReceiveKey()
    {
        ...
        CHECK(m_received.size() >= EllSwiftPubKey::size());
    ...
    std::vector<uint8_t> ReceivePacket(std::span<const std::byte> aad = {})
    {
        ...
        while (true) {
            ...
            CHECK(m_received.size() >= BIP324Cipher::LENGTH_LEN);
            ...
            CHECK(m_received.size() >= size + BIP324Cipher::EXPANSION);
    ...
    void ReceiveGarbage()
    {
        ...
        for (garblen = 0; garblen <= V2Transport::MAX_GARBAGE_LEN; ++garblen) {
            CHECK(m_received.size() >= garblen + BIP324Cipher::GARBAGE_TERMINATOR_LEN);
    ...
    {
        auto ret = ReceivePacket();
        CHECK(ret.size() == payload.size() + 1 + CMessageHeader::MESSAGE_TYPE_SIZE);
        CHECK(ret[0] == 0);
...
BOOST_AUTO_TEST_CASE(v2transport_test)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        ret = tester.Interact();
        CHECK(ret && ret->size() == 3);
        CHECK((*ret)[0] && (*ret)[0]->m_type == "cmpctblock" && std::ranges::equal((*ret)[0]->m_recv, MakeByteSpan(msg_data_1)));
        CHECK(!(*ret)[1]);
        CHECK((*ret)[2] && (*ret)[2]->m_type == "tx" && std::ranges::equal((*ret)[2]->m_recv, MakeByteSpan(msg_data_2)));
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        ret = tester.Interact();
        CHECK(ret && ret->size() == 2);
        CHECK((*ret)[0] && (*ret)[0]->m_type == "inv" && std::ranges::equal((*ret)[0]->m_recv, MakeByteSpan(msg_data_1)));
        CHECK((*ret)[1] && (*ret)[1]->m_type == "pong" && std::ranges::equal((*ret)[1]->m_recv, MakeByteSpan(msg_data_2)));
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        ret = tester.Interact();
        CHECK(ret && ret->size() == 4);
        CHECK((*ret)[0] && (*ret)[0]->m_type == "addrv2" && std::ranges::equal((*ret)[0]->m_recv, MakeByteSpan(msg_data_1)));
        CHECK((*ret)[1] && (*ret)[1]->m_type == "headers" && std::ranges::equal((*ret)[1]->m_recv, MakeByteSpan(msg_data_2)));
        CHECK(!(*ret)[2]);
        CHECK((*ret)[3] && (*ret)[3]->m_type == "foobar" && (*ret)[3]->m_recv.empty());
    ...
    // Send garbage that includes the first 15 garbage terminator bytes somewhere.
    {
        ...
        CHECK(ret && ret->size() == 2);
        CHECK(!(*ret)[0]);
        CHECK((*ret)[1] && (*ret)[1]->m_type == "block" && std::ranges::equal((*ret)[1]->m_recv, MakeByteSpan(msg_data_1)));
...
BOOST_AUTO_TEST_CASE(private_broadcast_version_does_not_update_addrman_services)
{
    ...
    CHECK(m_node.addrman->Add({addr}, source));
    CNode node{/*id=*/0,
               /*sock=*/nullptr,
               /*addrIn=*/addr,
               /*nKeyedNetGroupIn=*/0,
