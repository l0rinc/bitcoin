...
BOOST_AUTO_TEST_CASE(test_LogEscapeMessage)
{
    ...
    BOOST_CHECK_EQUAL(BCLog::LogEscapeMessage("Valid log message貓", /*allow_newlines=*/false), "Valid log message貓");
    // Control characters, including newlines, are escaped in C syntax.
    BOOST_CHECK_EQUAL(BCLog::LogEscapeMessage("Message\n with newlines\n", /*allow_newlines=*/false), R"(Message\x0a with newlines\x0a)");
    BOOST_CHECK_EQUAL(BCLog::LogEscapeMessage("\x01\x7f Corrupted log message\x0d", /*allow_newlines=*/false), R"(\x01\x7f Corrupted log message\x0d)");
    // Newlines can be preserved explicitly.
    BOOST_CHECK_EQUAL(BCLog::LogEscapeMessage("Message\n with newlines\n", /*allow_newlines=*/true), "Message\n with newlines\n");
    // Embedded NULL characters are escaped too.
    const std::string NUL("O\x00O", 3);
    BOOST_CHECK_EQUAL(BCLog::LogEscapeMessage(NUL, /*allow_newlines=*/false), R"(O\x00O)");
}
