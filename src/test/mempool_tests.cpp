...
BOOST_AUTO_TEST_CASE(MempoolTrimToSizeNoSpendsRemaining)
{
    CTxMemPool& pool = *Assert(m_node.mempool);
    LOCK2(cs_main, pool.cs);
    TestMemPoolEntryHelper entry;

    const CTransactionRef external = make_tx({10 * COIN});
    const CTransactionRef parent = make_tx({9 * COIN}, {external});
    const CTransactionRef child = make_tx({8 * COIN}, {parent});
    const CTransactionRef keeper_source = make_tx({11 * COIN});
    const CTransactionRef keeper = make_tx({9 * COIN}, {keeper_source});

    TryAddToMempool(pool, entry.Fee(10000).FromTx(keeper));
    const size_t keeper_usage = pool.DynamicMemoryUsage();
    TryAddToMempool(pool, entry.Fee(1000).FromTx(parent));
    TryAddToMempool(pool, entry.Fee(1000).FromTx(child));
    BOOST_REQUIRE(pool.exists(parent->GetHash()));
    BOOST_REQUIRE(pool.exists(child->GetHash()));
    BOOST_REQUIRE_EQUAL(pool.size(), 3U);
    BOOST_REQUIRE(pool.DynamicMemoryUsage() > keeper_usage);

    std::vector<COutPoint> no_spends_remaining;
    pool.TrimToSize(keeper_usage, &no_spends_remaining);

    BOOST_REQUIRE(pool.exists(keeper->GetHash()));
    BOOST_CHECK(!pool.exists(parent->GetHash()));
    BOOST_CHECK(!pool.exists(child->GetHash()));
    const std::set<COutPoint> expected{
        COutPoint{external->GetHash(), 0},
...
BOOST_FIXTURE_TEST_CASE(MempoolV1PreservesAggregateTotals, TestChain100Setup)
{
    ...
    const fs::path dump_path = m_path_root / "mempool-v1-aggregate-totals.dat";
    BOOST_REQUIRE(node::DumpMempool(source_pool, dump_path, fsbridge::fopen, true));
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastCount32PrefixPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-count32-prefix.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{
        std::byte{0xfe}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}};
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    {
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastCount64PrefixPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-count64-prefix.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastCountBodyPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-count-body.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    {
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(31, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
    {
        std::ofstream file{dump_path.std_path(), std::ios::binary};
        file.write(reinterpret_cast<const char*>(dump.data()), dump.size());
        file.close();
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidOneBytePreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-one-byte.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry{std::byte{0}};
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwoBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-two-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(2, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidThreeBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-three-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(3, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidFourBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-four-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(4, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidFiveBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-five-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(5, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidSixBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-six-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(6, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidSevenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-seven-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(7, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidEightBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-eight-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(8, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidNineBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-nine-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(9, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-ten-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(10, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidElevenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-eleven-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(11, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwelveBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twelve-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(12, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidThirteenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-thirteen-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(13, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidFourteenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-fourteen-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(14, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidFifteenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-fifteen-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(15, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidSixteenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-sixteen-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(16, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidSeventeenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-seventeen-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(17, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidEighteenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-eighteen-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(18, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidNineteenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-nineteen-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(19, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentyBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(20, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentyOneBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-one-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(21, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentyTwoBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-two-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(22, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentyThreeBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-three-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(23, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentyFourBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-four-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(24, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentyFiveBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-five-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(25, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentySixBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-six-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(26, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentySevenBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-seven-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(27, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentyEightBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-eight-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(28, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidTwentyNineBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-twenty-nine-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(29, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolTruncatedUnbroadcastTxidThirtyBytesPreservesState, TestChain100Setup)
{
    const CTransactionRef existing = MakeTransactionRef(CreateValidMempoolTransaction(
        m_coinbase_txns.front(), 0, 0, coinbaseKey, GetScriptForDestination(PKHash(coinbaseKey.GetPubKey())), 49 * COIN, false));
    CTxMemPool& destination = *Assert(m_node.mempool);
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-truncated-unbroadcast-txid-thirty-bytes.dat";
    DataStream dump;
    dump << uint64_t{2} << std::vector<std::byte>(Obfuscation::KEY_SIZE);
    const std::vector<std::byte> transaction_count{std::byte{0}};
    const std::vector<std::byte> map_count{std::byte{0}};
    const std::vector<std::byte> truncated_set_count{std::byte{1}};
    const std::vector<std::byte> truncated_set_entry(30, std::byte{0});
    dump.write(transaction_count);
    dump.write(map_count);
    dump.write(truncated_set_count);
    dump.write(truncated_set_entry);
...
BOOST_FIXTURE_TEST_CASE(MempoolV2DisabledMetadataOptions, TestChain100Setup)
{
    bilingual_str error;
    CTxMemPool::Options source_options{MemPoolOptionsForTest(m_node)};
    CTxMemPool source_pool{source_options, error};
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-disabled-metadata-options.dat";
    BOOST_REQUIRE(node::DumpMempool(source_pool, dump_path, fsbridge::fopen, true));
...
BOOST_FIXTURE_TEST_CASE(MempoolV2FeeOptionEnabledUnbroadcastDisabled, TestChain100Setup)
{
    bilingual_str error;
    CTxMemPool::Options source_options{MemPoolOptionsForTest(m_node)};
    CTxMemPool source_pool{source_options, error};
    ...
    const fs::path dump_path = m_path_root / "mempool-v2-fee-enabled-unbroadcast-disabled.dat";
    BOOST_REQUIRE(node::DumpMempool(source_pool, dump_path, fsbridge::fopen, true));
    ...
    BOOST_REQUIRE_EQUAL(destination.size(), 1U);
    BOOST_CHECK(destination.exists(tx->GetHash()));
    BOOST_CHECK_EQUAL(destination.info(tx->GetHash()).nFeeDelta, 7000);
    const auto deltas = destination.GetPrioritisedTransactions();
    BOOST_REQUIRE_EQUAL(deltas.size(), 1U);
    BOOST_CHECK(deltas.front().in_mempool);
    BOOST_CHECK_EQUAL(deltas.front().delta, 7000);
