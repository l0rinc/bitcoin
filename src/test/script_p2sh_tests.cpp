...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            if (i == j)
                CHECK_MESSAGE(sigOK, strprintf("VerifySignature %d %d", i, j));
            else
                CHECK_MESSAGE(!sigOK, strprintf("VerifySignature %d %d", i, j));
...
BOOST_AUTO_TEST_CASE(norecurse)
{
    ...
    CHECK_MESSAGE(err == SCRIPT_ERR_BAD_OPCODE, ScriptErrorString(err));
...
BOOST_AUTO_TEST_CASE(set)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        SignatureData empty;
        CHECK_MESSAGE(SignSignature(keystore, CTransaction(txFrom), txTo[i], 0, SIGHASH_ALL, empty), strprintf("SignSignature %d", i));
        CHECK_MESSAGE(IsStandardTx(CTransaction(txTo[i]), /*permit_bare_multisig=*/true, reason), strprintf("txTo[%d].IsStandard", i));
        bool no_pbms_is_std = IsStandardTx(CTransaction(txTo[i]), /*permit_bare_multisig=*/false, reason);
        CHECK_MESSAGE((i == 0 ? no_pbms_is_std : !no_pbms_is_std), strprintf("txTo[%d].IsStandard(permbaremulti=false)", i));
    }
}
...
BOOST_AUTO_TEST_CASE(switchover)
{
    ...
    CHECK_MESSAGE(err == SCRIPT_ERR_EQUALVERIFY, ScriptErrorString(err));
}
