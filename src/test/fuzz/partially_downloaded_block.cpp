...
FUZZ_TARGET(partially_downloaded_block, .init = initialize_pdb)
{
    ...
    // Select one rare collision mode directly so later modes do not require a long
    // run of false ConsumeBool results before they become reachable. Consume this
    // before deserialization, which may consume the entire remaining input.
    const uint8_t force_collision_mode{fuzzed_data_provider.ConsumeIntegralInRange<uint8_t>(0, 31)};
    ...
    const bool force_short_id_index_overflow{!force_invalid_init && !force_prefilled_short_id_collision && block->vtx.size() >= 2 && force_collision_mode == 0};
    if (force_short_id_index_overflow) {
        cmpctblock.FillShortTxIDs(std::numeric_limits<uint16_t>::max() + 2U);
    }

    const bool force_message_short_id_collision{!force_invalid_init && !force_prefilled_short_id_collision && !force_short_id_index_overflow && cmpctblock.ShortTxIDCount() >= 2 && force_collision_mode == 1};
    const bool force_short_id_collision{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() >= 3 && force_collision_mode == 2};
    const bool force_mempool_collision{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() >= 3 && force_collision_mode == 3};
    const bool force_mempool_extra_sequence{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() >= 3 && force_collision_mode == 4};
    const bool force_null_extra_collision{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() >= 3 && force_collision_mode == 5};
    const bool force_null_extra_after_extra_source{force_null_extra_collision && fuzzed_data_provider.ConsumeBool()};
    const bool force_duplicate_extra_collision{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() >= 3 && force_collision_mode == 6};
    const bool force_mempool_duplicate_then_collision{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() >= 3 && force_collision_mode == 7};
    const bool force_duplicate_extra_txn{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() >= 3 && force_collision_mode == 8};
    const bool force_mempool_early_exit_collision{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() == 3 && force_collision_mode == 9};
    const bool force_mempool_early_exit_collision_with_prefilled{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() == 4 && force_collision_mode == 10};
    const bool force_mempool_early_exit_collision_without_coinbase{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() == 3 && force_collision_mode == 11};
    const bool force_null_extra_after_mempool_completion{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() == 3 && force_collision_mode == 12};
    const bool force_extra_early_exit_collision{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() == 3 && force_collision_mode == 13};
    const bool force_extra_early_exit_collision_with_prefilled{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() == 4 && force_collision_mode == 14};
    const bool force_extra_early_exit_collision_without_coinbase{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() == 3 && force_collision_mode == 15};
    const bool force_independent_slot_collisions{!force_invalid_init && !force_valid_prefilled_tx && !force_prefilled_short_id_collision && block->vtx.size() == 3 && force_collision_mode == 16};
    if (force_extra_early_exit_collision_without_coinbase || force_mempool_early_exit_collision_without_coinbase) {
        ...
        cmpctblock.InsertCoinbaseShortTxID(coinbase_short_id);
    }
    if (force_mempool_early_exit_collision_with_prefilled) {
    ...
    bool forced_mempool_early_exit_collision_without_coinbase_applied{false};
    bool forced_null_extra_after_mempool_completion_applied{false};
    bool forced_extra_early_exit_collision_applied{false};
    bool forced_extra_early_exit_collision_with_prefilled_applied{false};
    bool forced_extra_early_exit_collision_without_coinbase_applied{false};
    bool forced_independent_slot_collisions_applied{false};
    ...
    if (force_short_id_index_overflow) {
    ...
    } else if (force_mempool_early_exit_collision_with_prefilled) {
        CMutableTransaction first_mempool_tx_mutable{*block->vtx[1]};
        first_mempool_tx_mutable.nLockTime ^= 1U;
        const CTransactionRef first_mempool_tx{MakeTransactionRef(std::move(first_mempool_tx_mutable))};
        CMutableTransaction second_mempool_tx_mutable{*block->vtx[3]};
        second_mempool_tx_mutable.nLockTime ^= 2U;
        const CTransactionRef second_mempool_tx{MakeTransactionRef(std::move(second_mempool_tx_mutable))};
        CMutableTransaction late_mempool_tx_mutable{*block->vtx[1]};
        late_mempool_tx_mutable.nLockTime ^= 4U;
        ...
        if (first_mempool_tx->GetWitnessHash() != second_mempool_tx->GetWitnessHash() &&
            ...
            if (pool.size() == 3) {
                const std::vector<uint64_t> target_short_ids{
                    cmpctblock.GetShortID(block->vtx[1]->GetWitnessHash()),
                    cmpctblock.GetShortID(block->vtx[3]->GetWitnessHash())};
                pdb.m_get_short_id_mock = [&forced_mempool_candidates_seen, target_short_ids, assigned_short_ids = std::vector<std::pair<Wtxid, uint64_t>>{}](const CBlockHeaderAndShortTxIDs& cmpctblock, const Wtxid& wtxid) mutable {
                    for (const auto& [assigned_wtxid, short_id] : assigned_short_ids) {
                        if (assigned_wtxid == wtxid) return short_id;
                    }
                    if (forced_mempool_candidates_seen < target_short_ids.size()) {
                        const uint64_t short_id{target_short_ids[forced_mempool_candidates_seen++]};
                        assigned_short_ids.emplace_back(wtxid, short_id);
                        return short_id;
                    }
                ...
                forced_mempool_early_exit_collision_with_prefilled_applied = true;
            }
        }
    } else if (force_mempool_early_exit_collision_without_coinbase) {
        CMutableTransaction first_mempool_tx_mutable{*block->vtx[1]};
        first_mempool_tx_mutable.nLockTime ^= 1U;
        const CTransactionRef first_mempool_tx{MakeTransactionRef(std::move(first_mempool_tx_mutable))};
        CMutableTransaction second_mempool_tx_mutable{*block->vtx[2]};
        second_mempool_tx_mutable.nLockTime ^= 2U;
        const CTransactionRef second_mempool_tx{MakeTransactionRef(std::move(second_mempool_tx_mutable))};
        CMutableTransaction third_mempool_tx_mutable{*block->vtx[1]};
        third_mempool_tx_mutable.nLockTime ^= 4U;
        const CTransactionRef third_mempool_tx{MakeTransactionRef(std::move(third_mempool_tx_mutable))};
        CMutableTransaction late_mempool_tx_mutable{*block->vtx[2]};
        late_mempool_tx_mutable.nLockTime ^= 8U;
        const CTransactionRef late_mempool_tx{MakeTransactionRef(std::move(late_mempool_tx_mutable))};

        if (first_mempool_tx->GetWitnessHash() != second_mempool_tx->GetWitnessHash() &&
            first_mempool_tx->GetWitnessHash() != third_mempool_tx->GetWitnessHash() &&
            first_mempool_tx->GetWitnessHash() != late_mempool_tx->GetWitnessHash() &&
            second_mempool_tx->GetWitnessHash() != third_mempool_tx->GetWitnessHash() &&
            second_mempool_tx->GetWitnessHash() != late_mempool_tx->GetWitnessHash() &&
            third_mempool_tx->GetWitnessHash() != late_mempool_tx->GetWitnessHash()) {
            TestMemPoolEntryHelper entry;
            for (const CTransactionRef& tx : {first_mempool_tx, second_mempool_tx, third_mempool_tx, late_mempool_tx}) {
                LOCK2(cs_main, pool.cs);
                TryAddToMempool(pool, entry.FromTx(tx));
            }
            if (pool.size() == 4) {
                ...
                forced_mempool_early_exit_collision_without_coinbase_applied = true;
            }
        }
    } else if (force_null_extra_after_mempool_completion) {
        CMutableTransaction first_mempool_tx_mutable{*block->vtx[1]};
        first_mempool_tx_mutable.nLockTime ^= 1U;
        const CTransactionRef first_mempool_tx{MakeTransactionRef(std::move(first_mempool_tx_mutable))};
        CMutableTransaction second_mempool_tx_mutable{*block->vtx[2]};
        second_mempool_tx_mutable.nLockTime ^= 2U;
        const CTransactionRef second_mempool_tx{MakeTransactionRef(std::move(second_mempool_tx_mutable))};
        CMutableTransaction extra_collision_tx_mutable{*block->vtx[1]};
        extra_collision_tx_mutable.nLockTime ^= 4U;
        const CTransactionRef extra_collision_tx{MakeTransactionRef(std::move(extra_collision_tx_mutable))};

        if (first_mempool_tx->GetWitnessHash() != second_mempool_tx->GetWitnessHash() &&
            first_mempool_tx->GetWitnessHash() != extra_collision_tx->GetWitnessHash() &&
            second_mempool_tx->GetWitnessHash() != extra_collision_tx->GetWitnessHash()) {
            TestMemPoolEntryHelper entry;
            ...
            if (pool.size() == 2) {
                ...
                extra_txn.emplace_back(extra_collision_wtxid, extra_collision_tx);
                pdb.m_get_short_id_mock = [&forced_mempool_candidates_seen, target_short_ids, empty_wtxid, extra_collision_wtxid, assigned_short_ids = std::vector<std::pair<Wtxid, uint64_t>>{}](const CBlockHeaderAndShortTxIDs&, const Wtxid& wtxid) mutable {
                    if (wtxid == empty_wtxid || wtxid == extra_collision_wtxid) return target_short_ids.front();
                    for (const auto& [assigned_wtxid, short_id] : assigned_short_ids) {
                        if (assigned_wtxid == wtxid) return short_id;
                    }
                    if (forced_mempool_candidates_seen < target_short_ids.size()) {
                ...
                forced_null_extra_after_mempool_completion_applied = true;
            }
        }
    ...
    } else if (force_extra_early_exit_collision_without_coinbase) {
        CMutableTransaction first_mempool_tx_mutable{*block->vtx[1]};
        first_mempool_tx_mutable.nLockTime ^= 1U;
        const CTransactionRef first_mempool_tx{MakeTransactionRef(std::move(first_mempool_tx_mutable))};
        CMutableTransaction first_extra_tx_mutable{*block->vtx[2]};
        first_extra_tx_mutable.nLockTime ^= 2U;
        const CTransactionRef first_extra_tx{MakeTransactionRef(std::move(first_extra_tx_mutable))};
        CMutableTransaction second_extra_tx_mutable{*block->vtx[1]};
        second_extra_tx_mutable.nLockTime ^= 4U;
        const CTransactionRef second_extra_tx{MakeTransactionRef(std::move(second_extra_tx_mutable))};
        CMutableTransaction late_extra_tx_mutable{*block->vtx[1]};
        late_extra_tx_mutable.nLockTime ^= 8U;
        const CTransactionRef late_extra_tx{MakeTransactionRef(std::move(late_extra_tx_mutable))};

        if (first_mempool_tx->GetWitnessHash() != first_extra_tx->GetWitnessHash() &&
            first_mempool_tx->GetWitnessHash() != second_extra_tx->GetWitnessHash() &&
            first_mempool_tx->GetWitnessHash() != late_extra_tx->GetWitnessHash() &&
            first_extra_tx->GetWitnessHash() != second_extra_tx->GetWitnessHash() &&
            first_extra_tx->GetWitnessHash() != late_extra_tx->GetWitnessHash() &&
            second_extra_tx->GetWitnessHash() != late_extra_tx->GetWitnessHash()) {
            TestMemPoolEntryHelper entry;
            {
                LOCK2(cs_main, pool.cs);
                TryAddToMempool(pool, entry.FromTx(first_mempool_tx));
            ...
            if (pool.size() == 1) {
                ...
                extra_txn.emplace_back(first_extra_wtxid, first_extra_tx);
                extra_txn.emplace_back(second_extra_wtxid, second_extra_tx);
                extra_txn.emplace_back(late_extra_wtxid, late_extra_tx);
                pdb.m_get_short_id_mock = [&forced_late_extra_seen, first_mempool_wtxid, first_extra_wtxid, second_extra_wtxid, late_extra_wtxid, first_short_id, second_short_id, third_short_id](const CBlockHeaderAndShortTxIDs& cmpctblock, const Wtxid& wtxid) {
                    if (wtxid == first_mempool_wtxid) return first_short_id;
                    if (wtxid == first_extra_wtxid) return second_short_id;
                ...
                forced_extra_early_exit_collision_without_coinbase_applied = true;
            }
        }
    ...
    if (force_independent_slot_collisions) {
        ...
        if (pool.size() == 2) {
            ...
            const Wtxid extra_a_wtxid{extra_a->GetWitnessHash()};
            const Wtxid extra_b_wtxid{extra_b->GetWitnessHash()};
            const uint64_t mempool_short_id{cmpctblock.GetShortID(block->vtx[1]->GetWitnessHash())};
            const uint64_t extra_short_id{cmpctblock.GetShortID(block->vtx[2]->GetWitnessHash())};
            extra_txn.emplace_back(extra_a_wtxid, extra_a);
            extra_txn.emplace_back(extra_b_wtxid, extra_b);
            pdb.m_get_short_id_mock = [mempool_a_wtxid, mempool_b_wtxid, extra_a_wtxid, extra_b_wtxid, mempool_short_id, extra_short_id](const CBlockHeaderAndShortTxIDs& cmpctblock, const Wtxid& wtxid) {
                if (wtxid == mempool_a_wtxid || wtxid == mempool_b_wtxid) return mempool_short_id;
                if (wtxid == extra_a_wtxid || wtxid == extra_b_wtxid) return extra_short_id;
                return cmpctblock.GetShortID(wtxid);
            };
            forced_independent_slot_collisions_applied = true;
        }
    } else if (force_mempool_collision) {
        CMutableTransaction first_mempool_tx_mutable{*block->vtx[1]};
        first_mempool_tx_mutable.nLockTime ^= 1U;
        const CTransactionRef first_mempool_tx{MakeTransactionRef(std::move(first_mempool_tx_mutable))};
        CMutableTransaction second_mempool_tx_mutable{*block->vtx[2]};
    ...
    if (forced_mempool_early_exit_collision_with_prefilled_applied) {
        assert(forced_mempool_candidates_seen == 2);
        assert(pdb.IsTxAvailable(1));
        assert(pdb.IsTxAvailable(2));
    ...
    if (forced_mempool_early_exit_collision_without_coinbase_applied) {
        assert(forced_mempool_candidates_seen == 3);
        assert(pdb.IsTxAvailable(0));
        assert(pdb.IsTxAvailable(1));
        assert(pdb.IsTxAvailable(2));
        assert(pdb.PrefilledCount() == 0);
        assert(pdb.MempoolCount() == 3);
        assert(pdb.ExtraCount() == 0);
    }
    if (forced_null_extra_after_mempool_completion_applied) {
        assert(forced_mempool_candidates_seen == 2);
        assert(pdb.IsTxAvailable(0));
        assert(!pdb.IsTxAvailable(1));
        assert(pdb.IsTxAvailable(2));
        assert(pdb.PrefilledCount() == 1);
        assert(pdb.MempoolCount() == 1);
        assert(pdb.ExtraCount() == 0);
    }
    ...
    if (forced_extra_early_exit_collision_without_coinbase_applied) {
        assert(!forced_late_extra_seen);
        assert(pdb.IsTxAvailable(0));
        assert(pdb.IsTxAvailable(1));
        assert(pdb.IsTxAvailable(2));
        assert(pdb.PrefilledCount() == 0);
        assert(pdb.MempoolCount() == 3);
        assert(pdb.ExtraCount() == 2);
    }
    if (forced_independent_slot_collisions_applied) {
        assert(!pdb.IsTxAvailable(1));
        assert(!pdb.IsTxAvailable(2));
        assert(pdb.PrefilledCount() == 1);
        assert(pdb.MempoolCount() == 0);
