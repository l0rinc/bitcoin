...
FUZZ_TARGET(netbase_dns_lookup)
{
...

    {
        ...
        for (const CNetAddr& resolved_address : resolved_addresses) {
            CHECK(!resolved_address.IsInternal());
        }
        CHECK(resolved_addresses.size() <= max_results || max_results == 0);
    }
    {
        ...
        if (resolved_address.has_value()) {
            CHECK(!resolved_address.value().IsInternal());
        }
    }
    {
        ...
        for (const CNetAddr& resolved_service : resolved_services) {
            CHECK(!resolved_service.IsInternal());
        }
        CHECK(resolved_services.size() <= max_results || max_results == 0);
    }
    {
        ...
        if (resolved_service.has_value()) {
            CHECK(!resolved_service.value().IsInternal());
        }
    }
    {
