    ...
    LIMITED_WHILE(provider.remaining_bytes() > 0, 500) {
        CallOneOf(
            ...
            [&] {
                ...
                const bool expected_inserted{!model.contains(key)};
                Assert(pb.Add(tx) == expected_inserted);
                if (expected_inserted) model.emplace(key, ModelTx{.tx = tx, .time_added = NodeClock::now(), .peers = {}});
            },
            [&] {
                const auto& tx{PickValue(provider, txs)};
                const auto key{Key(tx)};
