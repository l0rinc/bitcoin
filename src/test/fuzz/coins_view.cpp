...
void TestCoinsView(FuzzedDataProvider& fuzzed_data_provider, CCoinsView& backend_coins_view, bool is_db)
{
    ...
    LIMITED_WHILE(good_data && fuzzed_data_provider.ConsumeBool(), 10'000)
    {
        CallOneOf(
            ...
            [&] {
                CCoinsView* new_backend{fuzzed_data_provider.ConsumeBool() ? &backend_coins_view : &empty_backend};
                if (new_backend == &backend_coins_view && active_backend != &backend_coins_view) {
                    // Cached state is relative to the active backend. Drop it before
                    // restoring the original backend after using the empty backend.
                    {
                        const auto reset_guard{coins_view_cache.CreateResetGuard()};
                    }
                    if (is_db) coins_view_cache.SetBestBlock(uint256::ONE);
...

    {
        ...
        assert((active_backend == &backend_coins_view && is_db) == !!coins_view_cursor);
