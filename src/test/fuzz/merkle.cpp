...
FUZZ_TARGET(merkle)
{
    ...
    if (tx_hashes.size() > 1) {
        ...
        CHECK(merkle_root_from_merkle_path == merkle_root);
        CHECK(merkle_root_from_merkle_path == block_merkle_root);
    }

    // Basic sanity checks for TransactionMerklePath
    CHECK(merkle_path.size() <= 32); // Maximum depth of a Merkle tree with 2^32 leaves
    if (num_txs == 1 || num_txs == 0) {
        CHECK(merkle_path.empty()); // Single transaction has no path
    }
