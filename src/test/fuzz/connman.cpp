...
void AssertDistinctFuzzedSockMapKeys()
{
    ...
    auto sock_a{std::make_shared<FuzzedSock>(provider)};
    auto sock_b{std::make_shared<FuzzedSock>(provider)};
    Sock::EventsPerSock events;
    assert(events.emplace(sock_a, Sock::Events{Sock::RECV}).second);
    assert(events.emplace(sock_b, Sock::Events{Sock::SEND}).second);
}
