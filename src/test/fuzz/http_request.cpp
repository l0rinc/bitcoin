...
void AssertWriteReplyContracts(http_bitcoin::HTTPRequest& http_request, FuzzedDataProvider& fuzzed_data_provider, FakeSteadyClock& clock)
{
    ...
    const bool response_connection_before_close{fuzzed_data_provider.ConsumeBool()};
    ...
    if (response_close) {
        if (response_connection_before_close) {
...
void AssertParserStageAtomicityContracts(const std::string_view http_buffer)
{
...
void AssertReadRequestContracts(const std::string_view http_buffer)
{
    using http_bitcoin::HTTPRequest;
    using http_bitcoin::HTTPRemoteClient;
    using http_bitcoin::ContentTooLargeError;
    ...
    const std::string expected_remaining{recv_buffer_before.substr(direct_reader.Consumed())};
