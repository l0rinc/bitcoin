...
FUZZ_TARGET(integer, .init = initialize_integer)
{
    ...
    if (u64 <= MAX_MONEY) {
        ...
        CHECK(u64 == DecompressAmount(compressed_money_amount));
        static const uint64_t compressed_money_amount_max = CompressAmount(MAX_MONEY - 1);
        CHECK(compressed_money_amount <= compressed_money_amount_max);
    } else {
    ...
    (void)memusage::DynamicUsage(uch);
    {
        ...
        CHECK(dynamic_usage == incremental_dynamic_usage * i64s.size());
    }
    ...
    if (i32 >= 0 && i32 <= 16) {
        CHECK(i32 == CScript::DecodeOP_N(CScript::EncodeOP_N(i32)));
    }

    const std::chrono::seconds seconds{i64};
    CHECK(count_seconds(seconds) == i64);
    ...
    CHECK(ArithToUint256(au256) == u256);
    CHECK(uint256::FromHex(au256.GetHex()).value() == u256);
    (void)au256.bits();
...

    {
        ...
        CHECK(u32 == deserialized_u32be && stream.empty());
