    ...
    for (unsigned sim_idx = 1; sim_idx <= caches.size(); ++sim_idx) {
        ...
        for (uint32_t outpointidx = 0; outpointidx < NUM_OUTPOINTS; ++outpointidx) {
            ...
            if (overlay_fetch_scope) {
                ...
                assert_coin_matches_sim(real, sim);
            } else {
                const auto parent_cache_stats{cache_is_overlay[sim_idx - 1] ?
                    std::optional{get_cache_stats(sim_idx - 1)} :
                    std::optional<std::vector<CacheStats>>{}};
