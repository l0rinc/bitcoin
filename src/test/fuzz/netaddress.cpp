...
FUZZ_TARGET(netaddress)
{
    ...
    if (net_addr.GetNetwork() == Network::NET_IPV4) {
        CHECK(net_addr.IsIPv4());
    }
    if (net_addr.GetNetwork() == Network::NET_IPV6) {
        CHECK(net_addr.IsIPv6());
    }
    if (net_addr.GetNetwork() == Network::NET_ONION) {
        CHECK(net_addr.IsTor());
    }
    if (net_addr.GetNetwork() == Network::NET_I2P) {
        CHECK(net_addr.IsI2P());
    }
    if (net_addr.GetNetwork() == Network::NET_CJDNS) {
        CHECK(net_addr.IsCJDNS());
    }
    if (net_addr.GetNetwork() == Network::NET_INTERNAL) {
        CHECK(net_addr.IsInternal());
    }
    if (net_addr.GetNetwork() == Network::NET_UNROUTABLE) {
        CHECK(!net_addr.IsRoutable());
    }
    (void)net_addr.IsBindAny();
    if (net_addr.IsInternal()) {
        CHECK(net_addr.GetNetwork() == Network::NET_INTERNAL);
    }
    if (net_addr.IsIPv4()) {
        CHECK(net_addr.GetNetwork() == Network::NET_IPV4 || net_addr.GetNetwork() == Network::NET_UNROUTABLE);
    }
    if (net_addr.IsIPv6()) {
        CHECK(net_addr.GetNetwork() == Network::NET_IPV6 || net_addr.GetNetwork() == Network::NET_UNROUTABLE);
    }
    ...
    if (net_addr.IsRFC4193()) {
        CHECK(net_addr.GetNetwork() == Network::NET_INTERNAL || net_addr.GetNetwork() == Network::NET_UNROUTABLE);
    }
    ...
    if (!net_addr.IsRoutable()) {
        CHECK(net_addr.GetNetwork() == Network::NET_UNROUTABLE || net_addr.GetNetwork() == Network::NET_INTERNAL);
    }
    if (net_addr.IsTor()) {
        CHECK(net_addr.GetNetwork() == Network::NET_ONION);
    }
    if (net_addr.IsI2P()) {
        CHECK(net_addr.GetNetwork() == Network::NET_I2P);
    }
    if (net_addr.IsCJDNS()) {
        CHECK(net_addr.GetNetwork() == Network::NET_CJDNS);
    }
    ...
    CHECK((service == other_service) != (service != other_service));
    (void)(service < other_service);

    if (service.ToStringAddrPort() == other_service.ToStringAddrPort()) {
        CHECK(static_cast<CNetAddr>(service) == static_cast<CNetAddr>(other_service));
    }
    ...
    CHECK(net_addr == mutable_net_addr);
}
