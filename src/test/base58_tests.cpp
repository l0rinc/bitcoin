...
BOOST_AUTO_TEST_CASE(base58_DecodeBase58)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        std::string base58string = test[1].get_str();
        CHECK_MESSAGE(DecodeBase58(base58string, result, 256), strTest);
        CHECK_MESSAGE(result.size() == expected.size() && std::equal(result.begin(), result.end(), expected.begin()), strTest);
    }
