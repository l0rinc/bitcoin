...
BOOST_AUTO_TEST_CASE(max_tx_bip54_sigops)
{
    CCoinsViewCache coins{&CoinsViewEmpty::Get()};
    CKey key;
    key.MakeNewKey(true);

    // Run the consensus check with a fresh validation state, and pin the rejection details
    // whenever it fails.
    const auto check_bip54_sigops{[&coins](const CMutableTransaction& mtx) {
        TxValidationState state;
        CAmount fee;
        const bool ok{Consensus::CheckTxInputs(CTransaction(mtx), state, coins, /*nSpendHeight=*/0, fee, /*enforce_bip54=*/true)};
        ...
        if (!ok) {
            ...
            BOOST_CHECK_EQUAL(state.GetDebugMessage(), "too many legacy sigops (BIP54)");
        }
        return ok;
    }};
    ...
    BOOST_CHECK(check_bip54_sigops(tx_max_sigops));
