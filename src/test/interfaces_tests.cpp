...
BOOST_FIXTURE_TEST_CASE(findBlock, TestChain100Setup)
{
    ...
    CHECK(cur_active);
    CHECK(next_active);
    CHECK_EQUAL(next_hash, active[100]->GetBlockHash());
...
BOOST_FIXTURE_TEST_CASE(findFirstBlockWithTimeAndHeight, TestChain100Setup)
{
    ...
    CHECK_EQUAL(hash, active[5]->GetBlockHash());
    CHECK_EQUAL(height, std::remove_cvref_t<decltype(height)>{5});
...
BOOST_FIXTURE_TEST_CASE(findAncestorByHash, TestChain100Setup)
{
    ...
    CHECK_EQUAL(height, std::remove_cvref_t<decltype(height)>{10});
...
BOOST_FIXTURE_TEST_CASE(findCommonAncestor, TestChain100Setup)
{
    ...
    CHECK_EQUAL(active.Height(), orig_tip->nHeight - 10);
    ...
    CHECK_EQUAL(orig_height, orig_tip->nHeight);
    CHECK_EQUAL(fork_height, orig_tip->nHeight - 10);
    CHECK_EQUAL(fork_hash, active[fork_height]->GetBlockHash());
    ...
    CHECK_EQUAL(active_hash, active.Tip()->GetBlockHash());
    CHECK_EQUAL(orig_hash, orig_tip->GetBlockHash());
}
