...
void SimulationTest(CCoinsView* base, bool fake_best_block)
{
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        // Do a random modification.
        {
            ...
            bool result_havecoin = test_havecoin_before ? stack.back()->GetCoin(COutPoint(txid, 0)).has_value() : false;
            ...
            if (test_havecoin_after) {
                bool ret = stack.back()->GetCoin(COutPoint(txid, 0)).has_value();
        ...
        if (m_rng.randrange(1000) == 1 || i == NUM_SIMULATION_ITERATIONS - 1) {
            for (const auto& entry : result) {
                bool have = stack.back()->GetCoin(entry.first).has_value();
