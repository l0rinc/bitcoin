...
static std::ostream& operator<<(std::ostream& out, const Coin& coin)
{
    return out << "Coin{spent=" << coin.IsSpent() << ", coinbase=" << coin.fCoinBase
               << ", height=" << coin.nHeight << ", " << coin.out.ToString() << "}";
}
...
BOOST_FIXTURE_TEST_CASE(coins_db_leveldb_layout, FlushTest)
{
    ...
    BOOST_TEST(*Assert(base.GetCoin(outpoint)) == coin);
...
BOOST_AUTO_TEST_CASE(ccoins_addcoin_exception_keeps_usage_balanced)
{
    ...
    BOOST_TEST(cache.AccessCoin(outpoint) == coin1);
}
...
BOOST_AUTO_TEST_CASE(ccoins_reset_guard)
{
...

    {
        ...
        BOOST_TEST(cache.AccessCoin(outpoint) == coin);
