...
BOOST_AUTO_TEST_CASE(bip54_txsize)
{
    ...
    auto test_setup{TestingSetup{ChainType::REGTEST, {.extra_args = {"-vbparams=consensuscleanup:-1:-1"}}}};
    ...
    for (const auto& test_case: test_vectors) {
        ...
        auto block{node::BlockAssembler{chainman.ActiveChainstate(), /*mempool=*/nullptr, options}.CreateNewBlock()->block};
