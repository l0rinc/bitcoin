...
public:
    ...
    VersionBitsTester& TestStateSinceHeight(int height, int height_delayed)
    {
        ...
        for (int i = 0; i < CHECKERS; i++) {
            if (m_rng.randbits(i) == 0) {
                CHECK_MESSAGE(checker[i].StateSinceHeightFor(tip) == height, strprintf("Test %i for StateSinceHeight", num));
                CHECK_MESSAGE(checker_delayed[i].StateSinceHeightFor(tip) == height_delayed, strprintf("Test %i for StateSinceHeight (delayed)", num));
                CHECK_MESSAGE(checker_always[i].StateSinceHeightFor(tip) == 0, strprintf("Test %i for StateSinceHeight (always active)", num));
                CHECK_MESSAGE(checker_never[i].StateSinceHeightFor(tip) == 0, strprintf("Test %i for StateSinceHeight (never active)", num));
            }
        }
        num++;
        return *this;
    ...
    VersionBitsTester& TestState(ThresholdState exp, ThresholdState exp_delayed)
    {
        if (exp != exp_delayed) {
            ...
            CHECK_EQUAL(exp, ThresholdState::ACTIVE);
            CHECK_EQUAL(exp_delayed, ThresholdState::LOCKED_IN);
        }
        ...
        for (int i = 0; i < CHECKERS; i++) {
            if (m_rng.randbits(i) == 0) {
                ...
                CHECK_MESSAGE(got == exp, strprintf("Test %i for %s height %d (got %s)", num, StateName(exp), height, StateName(got)));
                CHECK_MESSAGE(got_delayed == exp_delayed, strprintf("Test %i for %s height %d (got %s; delayed case)", num, StateName(exp_delayed), height, StateName(got_delayed)));
                CHECK_MESSAGE(got_always == ThresholdState::ACTIVE, strprintf("Test %i for ACTIVE height %d (got %s; always active case)", num, height, StateName(got_always)));
                CHECK_MESSAGE(got_never == ThresholdState::FAILED, strprintf("Test %i for FAILED height %d (got %s; never active case)", num, height, StateName(got_never)));
            }
        }
        num++;
        return *this;
...
void check_computeblockversion(VersionBitsCache& versionbitscache, const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    CHECK(period > 0); // no division by zero, thankyou
    CHECK(0 < threshold); // must be able to have a window that doesn't activate
    CHECK(threshold < period); // must be able to have a window that does activate

    // should not be any signalling for first block
    CHECK_EQUAL(versionbitscache.ComputeBlockVersion(nullptr, params), VERSIONBITS_TOP_BITS);
        ...
        nStartTime == Consensus::BIP9Deployment::NEVER_ACTIVE)
    {
        ...
        CHECK_EQUAL(min_activation_height, std::remove_cvref_t<decltype(min_activation_height)>{0});
        CHECK_EQUAL(nTimeout, Consensus::BIP9Deployment::NO_TIMEOUT);
        return;
    }

    CHECK(nStartTime < nTimeout);
    CHECK(nStartTime >= 0);
    CHECK(nTimeout <= std::numeric_limits<uint32_t>::max() || nTimeout == Consensus::BIP9Deployment::NO_TIMEOUT);
    CHECK(0 <= bit && bit < 32);
    // Make sure that no deployment tries to set an invalid bit.
    CHECK(((1 << bit) & VERSIONBITS_TOP_MASK) == 0);
    CHECK(min_activation_height >= 0);
    // Check min_activation_height is on a retarget boundary
    CHECK_EQUAL(min_activation_height % period, 0U);
    ...
    if (nTime == 0) {
        ...
        CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit), std::remove_cvref_t<decltype(versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit))>{0});
    ...
    CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
    CHECK(!versionbitscache.IsActiveAfter(lastBlock, params, dep));
