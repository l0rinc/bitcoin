...
public:
    ...
    ThresholdState GetStateFor(const CBlockIndex* pindexPrev) const { return StateFor(pindexPrev); }
    int GetStateSinceHeightFor(const CBlockIndex* pindexPrev) const { return StateSinceHeightFor(pindexPrev); }
    void clear() { cache.clear(); }
};

static Consensus::BIP9Deployment TestDeployment(
...
BOOST_AUTO_TEST_CASE(versionbits_max_activation_height)
{
    ...
    TestConditionChecker checker{TestDeployment(/*max_activation_height=*/432)};
    ...
    TestConditionChecker checker2{TestDeployment(/*max_activation_height=*/1000)}; // max_activation_height far in future
...
BOOST_AUTO_TEST_CASE(versionbits_expired_state)
{
    ...
    TestConditionChecker checker{TestDeployment(/*max_activation_height=*/std::numeric_limits<int>::max(), /*active_duration=*/288)};
...
BOOST_AUTO_TEST_CASE(versionbits_expired_minimum_duration)
{
    ...
    TestConditionChecker checker{TestDeployment(/*max_activation_height=*/std::numeric_limits<int>::max(), /*active_duration=*/144)};
...
BOOST_AUTO_TEST_CASE(versionbits_expired_with_max_activation_height)
{
    ...
    TestConditionChecker checker{TestDeployment(/*max_activation_height=*/432, /*active_duration=*/288)};
...
BOOST_AUTO_TEST_CASE(versionbits_expired_zero_duration)
{
    ...
    TestConditionChecker checker{TestDeployment(/*max_activation_height=*/std::numeric_limits<int>::max(), /*active_duration=*/0)};
...
BOOST_AUTO_TEST_CASE(versionbits_no_signaling_after_expired)
{
    ...
    VersionBitsConditionChecker checker{params, dep};
    ThresholdConditionCache cache;
    const uint32_t bitmask = checker.Mask();
    const int period = params.vDeployments[dep].period; // 144 for regtest
    ...
    BOOST_CHECK(checker.GetStateFor(blocks.back(), cache) == ThresholdState::LOCKED_IN);
    ...
    for (int i = 0; i < period; i++) mine_block(VERSIONBITS_TOP_BITS | bitmask);
    BOOST_CHECK(checker.GetStateFor(blocks.back(), cache) == ThresholdState::ACTIVE);
    // ACTIVE: bit should NOT be set
    BOOST_CHECK_EQUAL(vbcache.ComputeBlockVersion(blocks.back(), params) & bitmask, 0u);

    // Period 3: ACTIVE (432-575)
    for (int i = 0; i < period; i++) mine_block(VERSIONBITS_TOP_BITS);
    BOOST_CHECK(checker.GetStateFor(blocks.back(), cache) == ThresholdState::EXPIRED);
