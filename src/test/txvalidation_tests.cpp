...
BOOST_FIXTURE_TEST_CASE(tx_mempool_rbf_conflicting_fees_saturate, TestChain100Setup)
{
    ...
    for (const auto& conflict : conflicts) {
        pool.PrioritiseTransaction(conflict->GetHash(), std::numeric_limits<CAmount>::min());
        LOCK(pool.cs);
        const auto entry{pool.GetIter(conflict->GetHash())};
        BOOST_REQUIRE(entry.has_value());
        BOOST_CHECK_EQUAL((*entry)->GetModifiedFee(), std::numeric_limits<CAmount>::min() + (*entry)->GetFee());
    }
    ...
    BOOST_CHECK_MESSAGE(result.m_result_type == MempoolAcceptResult::ResultType::INVALID, result.m_state.ToString());
    BOOST_CHECK_EQUAL(result.m_state.GetRejectReason(), "replacement-failed");
    BOOST_CHECK(result.m_replaced_transactions.empty());
