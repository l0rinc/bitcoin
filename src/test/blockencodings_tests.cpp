...
BOOST_AUTO_TEST_CASE(SimpleRoundTripTest)
{
    ...
    // Do a simple ShortTxIDs RT
    {
        ...
        CBlock block2;
        {
            ...
            BOOST_CHECK(partialBlock.FillBlock(block2, block2.GetHash(), {}, /*segwit_active=*/true) == READ_STATUS_INVALID); // No transactions
            partialBlock = tmp;
        }
        ...
        {
            PartiallyDownloadedBlock tmp = partialBlock;
            partialBlock.FillBlock(block2, block2.GetHash(), {block.vtx[2]}, /*segwit_active=*/true); // Current implementation doesn't check txn here, but don't require that
            partialBlock = tmp;
        }
        bool mutated;
        ...
        CBlock block3;
        BOOST_CHECK(partialBlock.FillBlock(block3, block3.GetHash(), {block.vtx[1]}, /*segwit_active=*/true) == READ_STATUS_OK);
...
BOOST_AUTO_TEST_CASE(NonCoinbasePreforwardRTTest)
{
    ...
    // Test with pre-forwarding tx 1, but not coinbase
    {
        ...
        {
            PartiallyDownloadedBlock tmp = partialBlock;
            partialBlock.FillBlock(block2, block2.GetHash(), {block.vtx[1]}, /*segwit_active=*/true); // Current implementation doesn't check txn here, but don't require that
            partialBlock = tmp;
        }
        ...
        BOOST_CHECK(partialBlock.FillBlock(block3, block3.GetHash(), {block.vtx[0]}, /*segwit_active=*/true) == READ_STATUS_OK);
        BOOST_CHECK_EQUAL(block.GetHash().ToString(), block3.GetHash().ToString());
        BOOST_CHECK_EQUAL(block.hashMerkleRoot.ToString(), BlockMerkleRoot(block3, &mutated).ToString());
        BOOST_CHECK(!mutated);
...
BOOST_AUTO_TEST_CASE(SufficientPreforwardRTTest)
{
    ...
    // Test with pre-forwarding coinbase + tx 2 with tx 1 in mempool
    {
        ...
        CBlock block2;
        PartiallyDownloadedBlock partialBlockCopy = partialBlock;
        BOOST_CHECK(partialBlock.FillBlock(block2, block2.GetHash(), {}, /*segwit_active=*/true) == READ_STATUS_OK);
...
BOOST_AUTO_TEST_CASE(EmptyBlockRoundTripTest)
{
    ...
    // Test simple header round-trip with only coinbase
    {
        ...
        BOOST_CHECK(partialBlock.FillBlock(block2, block2.GetHash(), vtx_missing, /*segwit_active=*/true) == READ_STATUS_OK);
        BOOST_CHECK_EQUAL(block.GetHash().ToString(), block2.GetHash().ToString());
        BOOST_CHECK_EQUAL(block.hashMerkleRoot.ToString(), BlockMerkleRoot(block2, &mutated).ToString());
        BOOST_CHECK(!mutated);
    }
