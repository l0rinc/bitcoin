...
BOOST_AUTO_TEST_CASE(package_hash_tests)
{
    ...
    CHECK_EQUAL(tx_1.GetWitnessHash(), wtxid_1);
    CHECK_EQUAL(tx_2.GetWitnessHash(), wtxid_2);
    CHECK_EQUAL(tx_3.GetWitnessHash(), wtxid_3);
    ...
    CHECK_EQUAL(tx_1.GetHash(), txid_1);
    CHECK_EQUAL(tx_2.GetHash(), txid_2);
    CHECK_EQUAL(tx_3.GetHash(), txid_3);
    ...
    CHECK_EQUAL(calculated_hash_123, GetPackageHash(package_123));
    CHECK_EQUAL(calculated_hash_123, GetPackageHash(package_132));
    CHECK_EQUAL(calculated_hash_123, GetPackageHash(package_231));
    CHECK_EQUAL(calculated_hash_123, GetPackageHash(package_213));
    CHECK_EQUAL(calculated_hash_123, GetPackageHash(package_312));
    CHECK_EQUAL(calculated_hash_123, GetPackageHash(package_321));
}
...
BOOST_AUTO_TEST_CASE(package_sanitization_tests)
{
    ...
    CHECK_EQUAL(state_too_many.GetResult(), PackageValidationResult::PCKG_POLICY);
    CHECK_EQUAL(state_too_many.GetRejectReason(), std::string_view{"package-too-many-transactions"});
    ...
    CHECK_EQUAL(state_too_large.GetResult(), PackageValidationResult::PCKG_POLICY);
    CHECK_EQUAL(state_too_large.GetRejectReason(), std::string_view{"package-too-large"});
    ...
    CHECK_EQUAL(state_duplicates.GetResult(), PackageValidationResult::PCKG_POLICY);
    CHECK_EQUAL(state_duplicates.GetRejectReason(), std::string_view{"package-contains-duplicates"});
    ...
    CHECK_EQUAL(state_conflicts.GetResult(), PackageValidationResult::PCKG_POLICY);
    CHECK_EQUAL(state_conflicts.GetRejectReason(), std::string_view{"conflict-in-package"});
...
BOOST_AUTO_TEST_CASE(package_validation_tests)
{
    ...
    if (auto err_parent_child{CheckPackageMempoolAcceptResult(package_parent_child, result_parent_child, /*expect_valid=*/true, nullptr)}) {
        CHECK_ERROR(err_parent_child.value());
    } else {
        ...
        CHECK_EQUAL(it_parent->second.m_wtxids_fee_calculations.value().size(), std::remove_cvref_t<decltype(it_parent->second.m_wtxids_fee_calculations.value().size())>{1});
        CHECK_EQUAL(it_parent->second.m_wtxids_fee_calculations.value().front(), tx_parent->GetWitnessHash());

        CHECK(it_child->second.m_effective_feerate.value().GetFee(GetVirtualTransactionSize(*tx_child)) == COIN);
        CHECK_EQUAL(it_child->second.m_wtxids_fee_calculations.value().size(), std::remove_cvref_t<decltype(it_child->second.m_wtxids_fee_calculations.value().size())>{1});
        CHECK_EQUAL(it_child->second.m_wtxids_fee_calculations.value().front(), tx_child->GetWitnessHash());
    }
    ...
    if (auto err_single_large{CheckPackageMempoolAcceptResult(package_single_giant, result_single_large, /*expect_valid=*/false, nullptr)}) {
        CHECK_ERROR(err_single_large.value());
    } else {
        CHECK_EQUAL(result_single_large.m_state.GetResult(), PackageValidationResult::PCKG_TX);
        CHECK_EQUAL(result_single_large.m_state.GetRejectReason(), std::string_view{"transaction failed"});
        auto it_giant_tx = result_single_large.m_tx_results.find(giant_ptx->GetWitnessHash());
        CHECK_EQUAL(it_giant_tx->second.m_state.GetRejectReason(), std::string_view{"tx-size"});
    }

    // Check that mempool size hasn't changed.
    CHECK_EQUAL(m_node.mempool->size(), initialPoolSize);
}
...
BOOST_AUTO_TEST_CASE(noncontextual_package_tests)
{
    ...
    // Parent and Child Package
    {
        ...
        CHECK_EQUAL(state.GetResult(), PackageValidationResult::PCKG_POLICY);
        CHECK_EQUAL(state.GetRejectReason(), std::string_view{"package-not-sorted"});
...
BOOST_AUTO_TEST_CASE(package_submission_tests)
{
    ...
    CHECK_EQUAL(result_unrelated_submit.m_state.GetResult(), PackageValidationResult::PCKG_POLICY);
    CHECK_EQUAL(result_unrelated_submit.m_state.GetRejectReason(), std::string_view{"package-not-child-with-parents"});
    CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
    ...
    // 3 Generations is not allowed.
    {
        ...
        CHECK_EQUAL(result_3gen_submit.m_state.GetResult(), PackageValidationResult::PCKG_POLICY);
        CHECK_EQUAL(result_3gen_submit.m_state.GetRejectReason(), std::string_view{"package-not-child-with-parents"});
        CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
    }
    ...
    // missing inputs, so the package validation isn't expected to happen.
    {
        ...
        if (auto err_parent_invalid{CheckPackageMempoolAcceptResult(package_invalid_parent, result_quit_early, /*expect_valid=*/false, m_node.mempool.get())}) {
            CHECK_ERROR(err_parent_invalid.value());
        } else {
            ...
            CHECK_EQUAL(it_parent->second.m_state.GetResult(), TxValidationResult::TX_WITNESS_MUTATED);
            CHECK_EQUAL(it_parent->second.m_state.GetRejectReason(), std::string_view{"bad-witness-nonstandard"});
            CHECK_EQUAL(it_child->second.m_state.GetResult(), TxValidationResult::TX_MISSING_INPUTS);
            CHECK_EQUAL(it_child->second.m_state.GetRejectReason(), std::string_view{"bad-txns-inputs-missingorspent"});
        }
        CHECK_EQUAL(result_quit_early.m_state.GetResult(), PackageValidationResult::PCKG_TX);
    }
    ...
    // Submit package with parent + child.
    {
        ...
        CHECK_MESSAGE(submit_parent_child.m_state.IsValid(),
                            "Package validation unexpectedly failed: " << submit_parent_child.m_state.GetRejectReason());
        CHECK_EQUAL(submit_parent_child.m_tx_results.size(), package_parent_child.size());
    ...
    // In-mempool parent and child with missing parent.
    {
        ...
        if (auto err_missing_parent{CheckPackageMempoolAcceptResult(package_missing_parent, result_missing_parent, /*expect_valid=*/false, m_node.mempool.get())}) {
            CHECK_ERROR(err_missing_parent.value());
        } else {
            ...
            CHECK_EQUAL(result_missing_parent.m_state.GetResult(), PackageValidationResult::PCKG_TX);
            CHECK_EQUAL(result_missing_parent.m_state.GetRejectReason(), std::string_view{"transaction failed"});

            CHECK_EQUAL(it_parent->second.m_state.GetResult(), TxValidationResult::TX_RECONSIDERABLE);
            CHECK_EQUAL(it_child->second.m_state.GetResult(), TxValidationResult::TX_MISSING_INPUTS);
            CHECK_EQUAL(it_child->second.m_state.GetRejectReason(), std::string_view{"bad-txns-inputs-missingorspent"});
            CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
        }
        ...
        if (auto err_parent_already_in{CheckPackageMempoolAcceptResult(package_missing_parent, result_parent_already_in, /*expect_valid=*/true, m_node.mempool.get())}) {
            CHECK_ERROR(err_parent_already_in.value());
        }
        CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
    }
}
...
BOOST_AUTO_TEST_CASE(package_single_tx)
{
    ...
    expected_pool_size += 1;
    CHECK_MESSAGE(result_single_tx.m_state.IsValid(),
    ...
    if (auto err_parent_child{CheckPackageMempoolAcceptResult(package_just_parent, result_just_parent, /*expect_valid=*/true, nullptr)}) {
        ...
        CHECK_MESSAGE(it_parent->second.m_state.IsValid(), it_parent->second.m_state.ToString());
    ...
    if (auto err_parent_child{CheckPackageMempoolAcceptResult(package_just_child, result_just_child, /*expect_valid=*/true, nullptr)}) {
        ...
        CHECK_MESSAGE(it_child->second.m_state.IsValid(), it_child->second.m_state.ToString());
    ...
    CHECK_EQUAL(result_single_tx_low_fee.m_state.GetResult(), PackageValidationResult::PCKG_TX);
    auto it_low_fee = result_single_tx_low_fee.m_tx_results.find(tx_single_low_fee->GetWitnessHash());
    CHECK_EQUAL(it_low_fee->second.m_state.GetResult(), TxValidationResult::TX_RECONSIDERABLE);
...
BOOST_AUTO_TEST_CASE(package_witness_swap_tests)
{
    ...
    CHECK_EQUAL(ptx_child1->GetHash(), ptx_child2->GetHash());
    ...
    // same-txid-different-witness.
    {
        ...
        if (auto err_witness1{CheckPackageMempoolAcceptResult(package_parent_child1, submit_witness1, /*expect_valid=*/true, m_node.mempool.get())}) {
            CHECK_ERROR(err_witness1.value());
        }
        ...
        if (auto err_witness2{CheckPackageMempoolAcceptResult(package_parent_child2, submit_witness2, /*expect_valid=*/true, m_node.mempool.get())}) {
            CHECK_ERROR(err_witness2.value());
        } else {
            ...
            CHECK_EQUAL(ptx_child1->GetWitnessHash(), it_child2->second.m_other_wtxid.value());
        }
        ...
        if (auto err_segwit_dedup{CheckPackageMempoolAcceptResult(package_parent_child1, submit_segwit_dedup, /*expect_valid=*/true, m_node.mempool.get())}) {
            CHECK_ERROR(err_segwit_dedup.value());
        } else {
    ...
    // We already submitted child1 above.
    {
        ...
        if (auto err_spend_ignored{CheckPackageMempoolAcceptResult(package_child2_grandchild, submit_spend_ignored, /*expect_valid=*/true, m_node.mempool.get())}) {
            CHECK_ERROR(err_spend_ignored.value());
        } else {
    ...
    // child should be accepted
    {
        ...
        if (auto err_mixed{CheckPackageMempoolAcceptResult(package_mixed, mixed_result, /*expect_valid=*/true, m_node.mempool.get())}) {
            ...
            CHECK_EQUAL(ptx_parent2_v2->GetWitnessHash(), it_parent2->second.m_other_wtxid.value());
...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    m_node.mempool->PrioritiseTransaction(tx_parent->GetHash(), child_value - coinbase_value);
    {
        ...
        if (auto err_cpfp_deprio{CheckPackageMempoolAcceptResult(package_cpfp, submit_cpfp_deprio, /*expect_valid=*/false, m_node.mempool.get())}) {
            CHECK_ERROR(err_cpfp_deprio.value());
        } else {
            CHECK_EQUAL(submit_cpfp_deprio.m_state.GetResult(), PackageValidationResult::PCKG_TX);
            CHECK_EQUAL(submit_cpfp_deprio.m_tx_results.find(tx_parent->GetWitnessHash())->second.m_state.GetResult(),
                              TxValidationResult::TX_RECONSIDERABLE);
            CHECK_EQUAL(submit_cpfp_deprio.m_tx_results.find(tx_child->GetWitnessHash())->second.m_state.GetResult(),
                              TxValidationResult::TX_RECONSIDERABLE);
            CHECK_EQUAL(submit_cpfp_deprio.m_tx_results.find(tx_parent->GetWitnessHash())->second.m_state.GetRejectReason(), std::string_view{"mempool min fee not met"});
            CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
        }
    }
    ...
    // Cheap package should fail for being too low fee.
    {
        ...
        if (auto err_package_too_low{CheckPackageMempoolAcceptResult(package_still_too_low, submit_package_too_low, /*expect_valid=*/false, m_node.mempool.get())}) {
            CHECK_ERROR(err_package_too_low.value());
        } else {
            // Individual feerate of parent is too low.
            CHECK_EQUAL(submit_package_too_low.m_tx_results.at(tx_parent_cheap->GetWitnessHash()).m_state.GetResult(),
                              TxValidationResult::TX_RECONSIDERABLE);
            ...
            CHECK_EQUAL(submit_package_too_low.m_tx_results.at(tx_child_cheap->GetWitnessHash()).m_state.GetResult(),
                              TxValidationResult::TX_RECONSIDERABLE);
        ...
        CHECK_EQUAL(submit_package_too_low.m_state.GetResult(), PackageValidationResult::PCKG_TX);
        CHECK_EQUAL(submit_package_too_low.m_state.GetRejectReason(), std::string_view{"transaction failed"});
        CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
    }
    ...
    // Now that the child's fees have "increased" by 1 BTC, the cheap package should succeed.
    {
        ...
        if (auto err_prioritised{CheckPackageMempoolAcceptResult(package_still_too_low, submit_prioritised_package, /*expect_valid=*/true, m_node.mempool.get())}) {
            CHECK_ERROR(err_prioritised.value());
        } else {
            ...
            CHECK_EQUAL(submit_prioritised_package.m_tx_results.size(), package_still_too_low.size());
    ...
    // Parent pays 1 BTC and child pays none. The parent should be accepted without the child.
    {
        ...
        if (auto err_rich_parent{CheckPackageMempoolAcceptResult(package_rich_parent, submit_rich_parent, /*expect_valid=*/false, m_node.mempool.get())}) {
            CHECK_ERROR(err_rich_parent.value());
        } else {
            // The child would have been validated on its own and failed.
            CHECK_EQUAL(submit_rich_parent.m_state.GetResult(), PackageValidationResult::PCKG_TX);
            CHECK_EQUAL(submit_rich_parent.m_state.GetRejectReason(), std::string_view{"transaction failed"});
            ...
            CHECK_MESSAGE(it_parent->second.m_base_fees.value() == high_parent_fee,
            ...
            CHECK_EQUAL(it_child->second.m_result_type, MempoolAcceptResult::ResultType::INVALID);
            CHECK_EQUAL(it_child->second.m_state.GetResult(), TxValidationResult::TX_RECONSIDERABLE);
            CHECK_EQUAL(it_child->second.m_state.GetRejectReason(), std::string_view{"mempool min fee not met"});
        }
        expected_pool_size += 1;
        CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
    }
...
BOOST_AUTO_TEST_CASE(package_rbf_tests)
{
    ...
    // Test that de-duplication works. This is not actually package rbf.
    {
        ...
        CHECK_EQUAL(it_parent_1->second.m_result_type, MempoolAcceptResult::ResultType::VALID);
        CHECK_EQUAL(it_child_1->second.m_result_type, MempoolAcceptResult::ResultType::VALID);
        expected_pool_size += 2;
        CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
        ...
        CHECK_EQUAL(it_parent_2->second.m_result_type, MempoolAcceptResult::ResultType::MEMPOOL_ENTRY);
        CHECK_EQUAL(it_child_2->second.m_result_type, MempoolAcceptResult::ResultType::VALID);
        CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
    ...
    // Test package rbf.
    {
        ...
        CHECK(tx_parent_1->GetHash() != tx_parent_2->GetHash() && tx_parent_2->GetHash() != tx_parent_3->GetHash());
        ...
        auto it_child_2 = submit2.m_tx_results.find(tx_child_2->GetWitnessHash());
        CHECK_EQUAL(it_parent_2->second.m_result_type, MempoolAcceptResult::ResultType::VALID);
        CHECK_EQUAL(it_child_2->second.m_result_type, MempoolAcceptResult::ResultType::VALID);
        CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
        ...
        CHECK_EQUAL(it_parent_3->second.m_result_type, MempoolAcceptResult::ResultType::VALID);
        CHECK_EQUAL(it_child_3->second.m_result_type, MempoolAcceptResult::ResultType::VALID);
        ...
        CHECK_EQUAL(it_parent_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), 199 + 1300);
        CHECK_EQUAL(it_child_3->second.m_effective_feerate.value().GetFee(package3_total_vsize), 199 + 1300);
