...
BOOST_AUTO_TEST_CASE(muhash_tests)
{
    ...
    MuHash3072 finalized_acc = FromInt(0);
    finalized_acc *= FromInt(1);
    finalized_acc /= FromInt(2);
    MuHash3072 unfinalized_acc{finalized_acc};

    uint256 finalized_once;
    uint256 finalized_twice;
    finalized_acc.Finalize(finalized_once);
    finalized_acc.Finalize(finalized_twice);
    BOOST_CHECK_EQUAL(finalized_once, finalized_twice);

    finalized_acc *= FromInt(3);
    unfinalized_acc *= FromInt(3);
    uint256 finalized_then_multiply;
    uint256 unfinalized_then_multiply;
    finalized_acc.Finalize(finalized_then_multiply);
    unfinalized_acc.Finalize(unfinalized_then_multiply);
    BOOST_CHECK_EQUAL(finalized_then_multiply, unfinalized_then_multiply);
    ...
    MuHash3072 fraction_serchk = FromInt(1);
    fraction_serchk /= FromInt(2);
    DataStream ss_fraction{};
    ss_fraction << fraction_serchk;
    MuHash3072 fraction_deserchk;
    ss_fraction >> fraction_deserchk;
    fraction_serchk.Finalize(out);
    fraction_deserchk.Finalize(out3);
    BOOST_CHECK_EQUAL(HexStr(out), HexStr(out3));
