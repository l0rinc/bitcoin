...
{
    // Real mainnet headers 0-7 (valid PoW). The on-disk nHeight is not
    // covered by the block hash, so a corrupt index can permute heights
    // while keeping every header genuine; loading such an index crashed in
    // CBlockIndex::BuildSkip()/GetAncestor() instead of failing gracefully.
    ...
    // Store genesis at height 0, blocks 3-7 at heights 1-5, block 1 at
    // height 6 and block 2 at height 7: contiguous, but every parent/child
    // height relation is broken.
    static const int PERMUTED_HEIGHT[8]{0, 6, 7, 1, 2, 3, 4, 5};
