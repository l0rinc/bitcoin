...
BOOST_AUTO_TEST_CASE(blockmanager_flush_block_file)
{
    ...
    constexpr unsigned FAST_PRUNE_CHUNK_SIZE{0x4000};
    ...
    // Fill the first block file with larger blocks until one lands in the next file
    CMutableTransaction big_tx;
    big_tx.vin.emplace_back().scriptSig.resize(FAST_PRUNE_CHUNK_SIZE / 2);
    block3.vtx.push_back(MakeTransactionRef(big_tx));
    ...
    BOOST_CHECK(info->nSize % FAST_PRUNE_CHUNK_SIZE != 0);
    BOOST_CHECK_EQUAL(fs::file_size(blockman.GetBlockPosFilename({0, 0})), info->nSize);
    BOOST_CHECK_EQUAL(fs::file_size(blockman.GetBlockPosFilename({1, 0})), FAST_PRUNE_CHUNK_SIZE);
