...
BOOST_AUTO_TEST_CASE(xor_roundtrip_random_chunks)
{
    ...
    for (size_t test{0}; test < 100; ++test) {
        ...
        const auto key_bytes{m_rng.randbool() ? m_rng.randbytes<sizeof(uint64_t)>() : std::array<std::byte, sizeof(uint64_t)>{}};
...
BOOST_AUTO_TEST_CASE(xor_bytes_reference)
{
    ...
    for (size_t test{0}; test < 100; ++test) {
        const size_t write_size{1 + m_rng.randrange(100U)};
        const size_t key_offset{m_rng.randrange(3 * sizeof(uint64_t))}; // Make sure the key can wrap around
...
BOOST_AUTO_TEST_CASE(buffered_reader_matches_autofile_random_content)
{
    ...
    const auto obfuscation{m_rng.randbytes<std::byte>(8)};
