...
BOOST_AUTO_TEST_CASE(xor_roundtrip_random_chunks)
{
    ...
    for (size_t test{0}; test < 100'000'000; ++test) {
...
{
    auto expected_xor{[](std::span<std::byte> target, const std::span<const std::byte, Obfuscation::SIZE_BYTES> obfuscation, size_t key_offset) {
        for (auto& b : target) {
...
BOOST_AUTO_TEST_CASE(xor_file)
{
...

    {
        ...
        AutoFile xor_file{raw_file("rb"), {key_bytes}}; // array-to-Obfuscation constructor
    ...
    uint64_t xor_key;
    std::memcpy(&xor_key, key_bytes.data(), Obfuscation::SIZE_BYTES);
    {
#ifdef __MINGW64__
        ...
        const char* mode = "wbx";
#endif
        AutoFile xor_file{raw_file(mode), xor_key}; // uint64_t-to-Obfuscation constructor
