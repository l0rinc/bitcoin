...
BOOST_AUTO_TEST_CASE(torcontrol_process_buffer_preserves_suffix)
{
    ...
    BOOST_CHECK_EQUAL(TorControlConnectionTest::ReceiveBuffer(conn), suffix);

    const std::string partial_before{TorControlConnectionTest::ReceiveBuffer(conn)};
    BOOST_CHECK(TorControlConnectionTest::ProcessBuffer(conn));
    BOOST_CHECK_EQUAL(TorControlConnectionTest::ReceiveBuffer(conn), partial_before);
}
...
BOOST_AUTO_TEST_CASE(torcontrol_process_buffer_rejects_excess_reply_lines)
{
    ...
    BOOST_CHECK_EQUAL(TorControlConnectionTest::ReceiveBuffer(conn), original_buffer);
}
