...
BOOST_FIXTURE_TEST_CASE(chainstatemanager, TestChain100Setup)
{
    ...
    BOOST_CHECK_EQUAL(&active_chain, &active_chainstate.m_chain);

    mineBlocks(10);
    BOOST_CHECK_EQUAL(WITH_LOCK(manager.GetMutex(), return manager.ActiveHeight()), 110);
    auto active_tip = WITH_LOCK(manager.GetMutex(), return manager.ActiveTip());
    auto exp_tip = active_chainstate.m_chain.Tip();
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_rebalance_caches, TestChain100Setup)
{
    ...
    Chainstate& chainstate = manager.ActiveChainstate();
    {
        LOCK(::cs_main);
        chainstate.InitCoinsCache(8_MiB);
        manager.MaybeRebalanceCaches();
    }

    BOOST_CHECK_EQUAL(chainstate.m_coinstip_cache_size_bytes, max_cache);
    BOOST_CHECK_EQUAL(chainstate.m_coinsdb_cache_size_bytes, max_cache);
}
