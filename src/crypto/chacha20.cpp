...
template <bool ADD>
ALWAYS_INLINE void ApplyInitialState(VectorState& x, std::span<const uint32_t, 12> state, uint32_t offset)
{
    Vec128 counters{};
    Vec128 nonces{};
...
ALWAYS_INLINE void ProcessBlocks(size_t& blocks, const std::byte*& in, std::byte*& out, std::span<uint32_t, 12> state)
{
    ...
    while (blocks >= BLOCKS_PER_CALL) {
        ...
        CryptBlocks<GROUPS>(in, out, state);
