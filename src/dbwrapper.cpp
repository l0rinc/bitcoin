...
static void SetMaxOpenFiles(leveldb::Options *options) {
    // On 64-bit Unix hosts, use a larger table cache so nodes with many small
    // chainstate files do not repeatedly evict and rebuild table-cache entries
    // during random UTXO reads. On 32-bit Unix hosts, decrease the value
    // because the handles use real fds and we want to avoid fd exhaustion
    // issues.
    //
