...
CCoinsViewDB::CCoinsViewDB(DBParams db_params, CoinsViewOptions options) :
    ...
    m_compacted_on_ibd_exit{m_db->Exists(DB_POST_IBD_COMPACTED)} { }
...
void CCoinsViewDB::CompactOnIbdExit()
{
    ...
    {
        LOCK(m_db_mutex);
        if (m_compacted_on_ibd_exit) return;

        LogInfo("Starting post-IBD chainstate database compaction of %s", path);
        m_db->CompactFull();
        m_db->Write(DB_POST_IBD_COMPACTED, true, /*fSync=*/true);
        m_compacted_on_ibd_exit = true;
    }
    LogInfo("Finished post-IBD chainstate database compaction of %s in %is", path, TicksSeconds(SteadyClock::now() - start));
}

bool CCoinsViewDB::NeedsCompactOnIbdExit()
{
    LOCK(m_db_mutex);
    return !m_compacted_on_ibd_exit;
}
