    ...
    m_presync_height(chain_start->nHeight)
{
    ...
    m_max_commitments = std::max<int64_t>(0, (6*(Ticks<std::chrono::seconds>(NodeClock::now() - NodeSeconds{std::chrono::seconds{chain_start->GetMedianTimePast()}}) + MAX_FUTURE_BLOCK_TIME) - m_headers_cache.capacity()) / HEADER_COMMITMENT_PERIOD);
...
bool HeadersSyncState::ValidateAndStoreRedownloadedHeader(const CBlockHeader& header, bool from_cache)
{
    ...
    if (header.hashPrevBlock != m_redownload_buffer_last_hash) {
        assert(!from_cache); // Cache should only contain contiguous headers.
