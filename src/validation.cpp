    ...
    DisconnectedBlockTransactions& disconnectpool)
{
    ...
    auto& vtx{block_to_connect->vtx};
    {
...

        {
            ...
            auto max_input_count{std::accumulate(std::next(vtx.cbegin()), vtx.cend(), size_t{0}, [](auto sum, const auto& tx) { return sum + tx->vin.size(); } )};
            std::vector<COutPoint> inputs;
            inputs.reserve(max_input_count);
            for (uint32_t i{1}; i < vtx.size(); ++i) {
                for (const auto& vin : vtx[i]->vin) {
                    if (!block_txids.contains(vin.prevout.hash)) {
                        inputs.push_back(vin.prevout);
                    }
                }
            ...
            for (const auto& op : inputs) {
                if (!cache.HaveCoinInCache(op)) {
                    if (auto coin{db_cache.GetCoin(op)}) {
                        block_cache.EmplaceCoinInternalDANGER(COutPoint{op}, std::move(*coin));
                    }
                }
            }
        }
