    ...
    try {
    {
        ...
        if (should_write) {
            ...
            if (!CoinsTip().GetBestBlock().IsNull()) {
                if (coins_mem_usage >= WARN_FLUSH_COINS_SIZE) LogWarning("Flushing large (%d GiB) UTXO set to disk, it may take several minutes", CeilDiv(coins_mem_usage, 1_GiB));
    ...
    const std::string& warning_messages) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    LogPrintLevel_(BCLog::LogFlags::ALL, BCLog::Level::Info, /*should_ratelimit=*/false, "%s%s: new best=%s height=%d version=0x%08x log2_work=%f tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)%s\n",
                   ...
                   coins_tip.DynamicMemoryUsage() * (1.0 / 1_MiB),
