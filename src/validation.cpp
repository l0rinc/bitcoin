                                       ...
                                       int64_t accept_time, bool bypass_limits, bool test_accept)
{
    ...
    if (result.m_result_type != MempoolAcceptResult::ResultType::VALID) {
        ...
        for (const COutPoint& hashTx : coins_to_uncache) {
            ...
            Assume(!active_chainstate.CoinsTip().HaveCoinInCache(hashTx));
        }
        ...
        if (should_write) {
            ...
            }
            {
                ...
                if (!m_blockman.FlushChainstateBlockFile(m_chain.Height())) {
                    ...
                    // FlushChainstateBlockFile has already notified the application
                    // about the I/O error. Do not publish metadata for unflushed data.
                    return state.Error("Failed to flush block file.");
                }
            }
...
bool Chainstate::PreciousBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    AssertLockNotHeld(::cs_main);
    {
        ...
        // previously invalid branches, and the active tip work may lag behind
        // higher-work candidates. Keep later preciousblock calls ordered ahead
        // of any earlier same-work priorities.
    ...
    const SnapshotMetadata& metadata)
{
    ...
    Assume(maybe_stats->nTransactionOutputs == maybe_stats->coins_count);
    Assume(maybe_stats->hashBlock == base_blockhash);
    Assume(maybe_stats->nHeight == base_height);
