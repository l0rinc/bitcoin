                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (auto& queue = m_chainman.GetCheckQueue(); queue.HasThreads() && assume_valid != AssumeValid::SKIPPED) control.emplace(queue);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase() && assume_valid != AssumeValid::SKIPPED)
        {
