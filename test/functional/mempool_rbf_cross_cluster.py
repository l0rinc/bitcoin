...
"""Test one replacement transaction spanning two independent mempool clusters."""
...
class MempoolRbfCrossClusterTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        funding_1 = wallet.create_self_transfer_multi(confirmed_only=True, fee_per_output=1000)
        funding_2 = wallet.create_self_transfer_multi(confirmed_only=True, fee_per_output=1000)
        source.sendrawtransaction(hexstring=funding_1["hex"])
        source.sendrawtransaction(hexstring=funding_2["hex"])

        conflict_1 = wallet.create_self_transfer_multi(
            utxos_to_spend=funding_1["new_utxos"], fee_per_output=1000)
        conflict_2 = wallet.create_self_transfer_multi(
            utxos_to_spend=funding_2["new_utxos"], fee_per_output=1000)
        ...
        expected_before = {
            funding_1["txid"], funding_2["txid"], conflict_1["txid"], conflict_2["txid"]
        }
        ...
        assert_equal(before_entries[funding_1["txid"]]["spentby"], [conflict_1["txid"]])
        assert_equal(before_entries[funding_2["txid"]]["spentby"], [conflict_2["txid"]])
        assert_equal(before_entries[conflict_1["txid"]]["depends"], [funding_1["txid"]])
        assert_equal(before_entries[conflict_2["txid"]]["depends"], [funding_2["txid"]])
        assert_equal(
        ...
        low_fee_replacement = wallet.create_self_transfer_multi(
            utxos_to_spend=[funding_1["new_utxos"][0], funding_2["new_utxos"][0]],
            fee_per_output=2400,
        )
        ...
        expected_after = {funding_1["txid"], funding_2["txid"], high_fee_replacement["txid"]}
        ...
        assert_equal(after_entries[funding_1["txid"]]["spentby"], [high_fee_replacement["txid"]])
        assert_equal(after_entries[funding_2["txid"]]["spentby"], [high_fee_replacement["txid"]])
        assert_equal(
            set(after_entries[high_fee_replacement["txid"]]["depends"]),
            {funding_1["txid"], funding_2["txid"]},
        )
        assert_equal(after_entries[funding_1["txid"]]["unbroadcast"], True)
        assert_equal(after_entries[funding_2["txid"]]["unbroadcast"], True)
...
if __name__ == "__main__":
    MempoolRbfCrossClusterTest(__file__).main()
