...
class AssumeValidBlockStore(P2PInterface):
    def __init__(self, blocks, max_height_to_serve, first_height=1):
        super().__init__()
        self.blocks = blocks
...
class FeaturePruneAssumeValidTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Reject restart without -pruneassumevalid after stripped witness-era blocks were connected")
        self.stop_node(1)
        self.nodes[1].assert_start_raises_init_error(
            extra_args=["-prune=1", f"-assumevalid={assumevalid_hash}", f"-minimumchainwork={final_chainwork}"],
            expected_msg="This chainstate contains witness-era blocks pruned by -pruneassumevalid. Please restart with -pruneassumevalid or rebuild the database using -reindex.\nPlease restart with -reindex or -reindex-chainstate to recover.",
        )
        self.start_node(1, extra_args=prune_assumevalid_args)
        prune_assumevalid_node = self.nodes[1]
