...
def chainstate_compaction_msgs(node):
    ...
    return [
        "Checking chainstate database compaction after leaving IBD",
        f"Starting post-IBD chainstate database compaction of {chainstate_path}",
        f"Finished post-IBD chainstate database compaction of {chainstate_path}",
    ]
...
class CompactChainstateTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        with node.assert_debug_log(
            ...
            unexpected_msgs=compaction_msgs + [force_flush_msg],
        ):
            self.generate(node, 1, sync_fun=self.no_op)
        assert_equal(node.getblockchaininfo()["initialblockdownload"], False)

        self.log.info("Drop post-IBD compaction marker during -reindex-chainstate")
        with node.assert_debug_log(compaction_msgs + ["Leaving InitialBlockDownload", force_flush_msg]):
            self.restart_node(0, extra_args=[*DEBUG_ARGS, "-reindex-chainstate"])
        assert_equal(node.getblockchaininfo()["initialblockdownload"], False)

        self.log.info("Drop post-IBD compaction marker during -reindex")
        with node.assert_debug_log(compaction_msgs + ["Leaving InitialBlockDownload", force_flush_msg]):
            self.restart_node(0, extra_args=[*DEBUG_ARGS, "-reindex"])
        assert_equal(node.getblockchaininfo()["initialblockdownload"], False)
