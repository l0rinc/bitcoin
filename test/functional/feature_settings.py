...
class SettingsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # The settings file path is selected by command-line/config options,
        # not by a value read from the settings file itself.
        redirected = node.chain_path / "redirected.json"
        if redirected.exists():
            redirected.unlink()
        with settings.open("w") as fp:
            json.dump({"settings": "redirected.json", "uacomment": "self-reference"}, fp)
        self.start_node(0)
        self.stop_node(0)
        with settings.open() as fp:
            assert_equal(json.load(fp), {
                ...
                "settings": "redirected.json",
                "uacomment": "self-reference",
            })
        assert_equal(redirected.exists(), False)
