...
When `--target-rpc` is set, the proxy forces each served stale block to become
preferred on the target node, waits for the target to reach the upstream tip,
then prints and compares the final `gettxoutsetinfo(hash_serialized_3)` result.
...
Missing reorg evidence causes a non-zero exit code.
...
"$SRC_DIR/contrib/historical_reorg_proxy.py" \
  ...
  --target-rpc=http://127.0.0.1:19454 \
  --target-cookie-file="$TGT_DATA_DIR/.cookie" \
...
When the target reaches the upstream tip, the proxy prints the final UTXO hash
comparison and reorg validation summary, then exits.
