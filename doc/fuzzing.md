...
The `bip54_sigops` target can be seeded from the JSON vectors generated by the
`bip54_tests` unit test. The corpus encoding preserves raw scriptSigs and
spent scriptPubKeys, including malformed P2SH edge cases. Temporarily
uncomment `UPDATE_JSON_TESTS` in `src/test/bip54_tests.cpp`, rebuild
`test_bitcoin`, and run:

```sh
$ cd /tmp
$ /path/to/bitcoin/build/bin/test_bitcoin --run_test=bip54_tests/bip54_legacy_sigops
$ cd /path/to/bitcoin
$ contrib/devtools/bip54_seed_corpus.py /tmp/bip54_sigops.json.gen /tmp/bip54_sigops
$ FUZZ=bip54_sigops build_fuzz/bin/fuzz /tmp/bip54_sigops
```
